...
> [!WARNING]
> インデックスの削除やスキーマのロールバックを行っても、**マイグレーションや重複解消スクリプトによって削除された行（データ）は復元されません**。
> 削除されたデータを復旧させるには、バックアップからリストアする必要があります。
> 再度マイグレーションや解消手順を実行する前に、必ず安全な環境でバックアップからのリストアを検証し、データの整合性を確認してください。
