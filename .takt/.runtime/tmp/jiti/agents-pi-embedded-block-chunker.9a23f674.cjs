"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.EmbeddedBlockChunker = void 0;var _fences = require("../markdown/fences.js");
class EmbeddedBlockChunker {
  ...
  #drainParagraphs(emit, maxChars) {
    while (this.#buffer.length > 0) {
      const fenceSpans = (0, _fences.parseFenceSpans)(this.#buffer);
  ...
  #pickSoftBreakIndex(buffer, minCharsOverride) {
    ...
    const fenceSpans = (0, _fences.parseFenceSpans)(buffer);
    ...
    if (preference === "paragraph") {
      ...
      while (paragraphIdx !== -1) {
        ...
        for (const candidate of candidates) {
          ...
          if ((0, _fences.isSafeFenceBreak)(fenceSpans, candidate)) {
    ...
    if (preference === "paragraph" || preference === "newline") {
      ...
      while (newlineIdx !== -1) {
        if (newlineIdx >= minChars && (0, _fences.isSafeFenceBreak)(fenceSpans, newlineIdx)) {
  ...
  #pickBreakIndex(buffer, minCharsOverride) {
    ...
    if (preference === "paragraph" || preference === "newline") {
      ...
      while (newlineIdx >= minChars) {
        if ((0, _fences.isSafeFenceBreak)(fenceSpans, newlineIdx)) {
          return { index: newlineIdx };
        }
    ...
    for (let i = window.length - 1; i >= minChars; i--) {
      if (/\s/.test(window[i]) && (0, _fences.isSafeFenceBreak)(fenceSpans, i)) {
        return { index: i };
      }
    }
    if (buffer.length >= maxChars) {
      if ((0, _fences.isSafeFenceBreak)(fenceSpans, maxChars)) {
        return { index: maxChars };
      }
      const fence = (0, _fences.findFenceSpanAt)(fenceSpans, maxChars);
      if (fence) {
        return {
          index: maxChars,
          fenceSplit: {
...
function findNextParagraphBreak(buffer, fenceSpans, startIndex = 0) {
  ...
  while ((match = re.exec(buffer)) !== null) {
    ...
    if (!(0, _fences.isSafeFenceBreak)(fenceSpans, index)) {
      continue;
    }
