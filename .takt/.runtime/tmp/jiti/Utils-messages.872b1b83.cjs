 ...
 */
const extractUrlFromText = (text) => text.match(_index2.URL_REGEX)?.[0];exports.extractUrlFromText = extractUrlFromText;
...
const prepareWAMessageMedia = async (message, options) => {
  ...
  if (!mediaType) {
    throw new _boom.Boom('Invalid media type', { statusCode: 400 });
  }
  const uploadData = {
    ...message,
  ...
  if (isNewsletter) {
    ...
    const { filePath, fileSha256, fileLength } = await (0, _messagesMedia.getRawMediaUploadData)(uploadData.media, options.mediaTypeOverride || mediaType, logger);
  ...
  const { mediaKey, encFilePath, originalFilePath, fileEncSha256, fileSha256, fileLength } = await (0, _messagesMedia.encryptedStream)(uploadData.media, options.mediaTypeOverride || mediaType, {
    logger,
  ...
  (async () => {
    try {
      if (requiresThumbnailComputation) {
        const { thumbnail, originalImageDimensions } = await (0, _messagesMedia.generateThumbnail)(originalFilePath, mediaType, options);
...
const generateForwardMessageContent = (message, forceForward) => {
  ...
  if (!content) {
    throw new _boom.Boom('no content in message', { statusCode: 400 });
  }
  // hacky copy
...
const generateWAMessageContent = async (message, options) => {
  ...
  if ('contacts' in message) {
    ...
    if (!contactLen) {
      throw new _boom.Boom('require atleast 1 contact', { statusCode: 400 });
    }
    if (contactLen === 1) {
  ...
  if ('poll' in message) {
    ...
    if (!Array.isArray(message.poll.values)) {
      throw new _boom.Boom('Invalid poll values', { statusCode: 400 });
    }
  ...
  if ('sharePhoneNumber' in message) {
    m.protocolMessage = {
      type: _index.proto.Message.ProtocolMessage.Type.SHARE_PHONE_NUMBER
    };
  } else
  if ('requestPhoneNumber' in message) {
    m.requestPhoneNumberMessage = {};
  } else
  if ('limitSharing' in message) {
    m.protocolMessage = {
...
const generateWAMessageFromContent = (jid, message, options) => {
  ...
  if (quoted && !(0, _index4.isJidNewsletter)(jid)) {
    ...
    quotedMsg = _index.proto.Message.create({ [msgType]: quotedMsg[msgType] });
  ...
  const messageJSON = {
    ...
    participant: (0, _index4.isJidGroup)(jid) || (0, _index4.isJidStatusBroadcast)(jid) ? userJid : undefined, // TODO: Add support for LIDs
...
/** Get the key to access the true type of content */exports.generateWAMessage = generateWAMessage;
...
'desktop' :
'unknown';
/** Upserts a receipt in the message */exports.getDevice = getDevice;
...
/** Update the message with a new reaction */exports.updateMessageWithReceipt = updateMessageWithReceipt;
...
/** Update the message with a new poll update */exports.updateMessageWithReaction = updateMessageWithReaction;
...
/** Update the message with a new event response */exports.updateMessageWithPollUpdate = updateMessageWithPollUpdate;
...
const downloadMediaMessage = async (message, type, options, ctx) => {
  ...
  async function downloadMsg() {
    ...
    if (!mContent) {
      throw new _boom.Boom('No message present', { statusCode: 400, data: message });
    }
...
/** Checks whether the given message is a media message; if it is returns the inner content */exports.downloadMediaMessage = downloadMediaMessage;
const assertMediaContent = (content) => {
  ...
  if (!mediaContent) {
    throw new _boom.Boom('given message is not a media message', { statusCode: 400, data: content });
  }
  return mediaContent;
};exports.assertMediaContent = assertMediaContent; /* v9-87c2ea91f6252e38 */
