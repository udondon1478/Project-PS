"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.Client = void 0;var _v = require("discord-api-types/v10");
var _channelFactory = require("../functions/channelFactory.js");
var _CommandHandler = require("../internals/CommandHandler.js");
var _ComponentHandler = require("../internals/ComponentHandler.js");
var _EmojiHandler = require("../internals/EmojiHandler.js");
var _EventHandler = require("../internals/EventHandler.js");
var _ModalHandler = require("../internals/ModalHandler.js");
var _TemporaryListenerManager = require("../internals/TemporaryListenerManager.js");
...
var _Webhook = require("../structures/Webhook.js");
var _index = require("../utils/index.js");
var _RequestClient = require("./RequestClient.js");
/**
...
class Client {
  ...
  constructor(options, handlers, plugins = []) {
    ...
    this.commandHandler = new _CommandHandler.CommandHandler(this);
    this.componentHandler = new _ComponentHandler.ComponentHandler(this);
    this.modalHandler = new _ModalHandler.ModalHandler(this);
    this.eventHandler = new _EventHandler.EventHandler(this);
    this.temporaryListeners = new _TemporaryListenerManager.TemporaryListenerManager(this);
    this.emoji = new _EmojiHandler.EmojiHandler(this);
    ...
    this.rest = new _RequestClient.RequestClient(options.token, options.requestOptions);
    this.appendRoutes();
  ...
  async handleDeployRequest() {
    ...
    if (this.options.devGuilds && this.options.devGuilds.length > 0) {
      for (const guildId of this.options.devGuilds) {
        const deployed = await this.rest.put(_v.Routes.applicationGuildCommands(this.options.clientId, guildId), { body: commands.map((c) => c.serialize()) });
    ...
    for (const [guildId, cmds] of Object.entries(guildCommandsMap)) {
      const deployed = await this.rest.put(_v.Routes.applicationGuildCommands(this.options.clientId, guildId), { body: cmds });
      this.updateCommandIdsFromDeployment(deployed);
    }
    // Deploy global commands
    if (globalCommands.length > 0) {
      const deployed = await this.rest.put(_v.Routes.applicationCommands(this.options.clientId), {
  ...
  async handleEventsRequest(req) {
    ...
    if (payload.type === _v.ApplicationWebhookType.Ping)
  ...
  async handleInteractionsRequest(req, ctx) {
    ...
    if (interaction.type === _v.InteractionType.Ping) {
      return Response.json({ type: _v.InteractionResponseType.Pong });
    }
  ...
  async handleInteraction(interaction, ctx) {
    if (interaction.type === _v.InteractionType.ApplicationCommand) {
      const promise = this.commandHandler.handleCommandInteraction(interaction);
      if (ctx?.waitUntil)
      ctx.waitUntil(promise);else

      await promise;
    }
    if (interaction.type === _v.InteractionType.ApplicationCommandAutocomplete) {
    ...
    if (interaction.type === _v.InteractionType.MessageComponent) {
    ...
    if (interaction.type === _v.InteractionType.ModalSubmit) {
  ...
  async validateDiscordRequest(req) {
    ...
    return false;
    try {
      const timestampData = (0, _index.valueToUint8Array)(timestamp);
      const bodyData = (0, _index.valueToUint8Array)(body);
      const message = (0, _index.concatUint8Arrays)(timestampData, bodyData);
      ...
      for (const publicKey of publicKeys) {
        try {
          const publicKeyBuffer = (0, _index.valueToUint8Array)(publicKey, "hex");
          const signatureBuffer = (0, _index.valueToUint8Array)(signature, "hex");
          ...
          const isValid = await _index.subtleCrypto.verify({
            name: "ed25519"
          }, await _index.subtleCrypto.importKey("raw", publicKeyArrayBuffer, {
            name: "ed25519",
            namedCurve: "ed25519"
   ...
   */
  async fetchUser(id) {
    const user = await this.rest.get(_v.Routes.user(id));
    return new _User.User(this, user);
  }
  /**
   ...
   */
  async fetchGuild(id) {
    const guild = await this.rest.get(_v.Routes.guild(id));
    return new _Guild.Guild(this, guild);
  }
  /**
  ...
  async fetchChannel(id) {
    const channel = await this.rest.get(_v.Routes.channel(id));
    return (0, _channelFactory.channelFactory)(this, channel);
  }
  /**
  ...
  async fetchRole(guildId, id) {
    const role = await this.rest.get(_v.Routes.guildRole(guildId, id));
    return new _Role.Role(this, role, guildId);
  }
  /**
  ...
  async fetchMember(guildId, id) {
    const member = await this.rest.get(_v.Routes.guildMember(guildId, id));
    return new _GuildMember.GuildMember(this, member, new _Guild.Guild(this, guildId));
  }
  /**
  ...
  async fetchMessage(channelId, messageId) {
    const message = await this.rest.get(_v.Routes.channelMessage(channelId, messageId));
    return new _Message.Message(this, message);
  }
  /**
  ...
  async fetchWebhook(input) {
    const webhook = new _Webhook.Webhook(input);
  ...
  async getDiscordCommands(force = false) {
    ...
    const commands = await this.rest.get(_v.Routes.applicationCommands(this.options.clientId));
...
}exports.Client = Client; /* v9-7b12cdb0d5f2607e */
