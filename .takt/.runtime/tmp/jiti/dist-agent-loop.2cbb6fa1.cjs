"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.agentLoop = agentLoop;exports.agentLoopContinue = agentLoopContinue;
 ...
 */function agentLoop(prompts, context, config, signal, streamFn) {const stream = createAgentStream();
  (async () => {
...
function createAgentStream() {
  return new _piAi.EventStream((event) => event.type === "agent_end", (event) => event.type === "agent_end" ? event.messages : []);
}
/**
...
async function streamAssistantResponse(context, config, signal, stream, streamFn) {
  ...
  const streamFunction = streamFn || _piAi.streamSimple;
...
async function executeToolCalls(tools, assistantMessage, signal, stream, getSteeringMessages) {
  ...
  for (let index = 0; index < toolCalls.length; index++) {
    ...
    try {
      ...
      const validatedArgs = (0, _piAi.validateToolArguments)(tool, toolCall);
