"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.applyGoogleTurnOrderingFix = applyGoogleTurnOrderingFix;exports.logToolSchemasForGoogle = logToolSchemasForGoogle;exports.onUnhandledCompactionFailure = onUnhandledCompactionFailure;exports.sanitizeSessionHistory = sanitizeSessionHistory;exports.sanitizeToolsForGoogle = sanitizeToolsForGoogle;var _nodeEvents = require("node:events");
...
function sanitizeToolsForGoogle(params) {
  ...
  return params.tools.map((tool) => {
    ...
    return {
      ...tool,
      parameters: (0, _piToolsSchema.cleanToolSchemaForGemini)(tool.parameters)
    };
  });
}
function logToolSchemasForGoogle(params) {
  ...
  _logger.log.info("google tool schema snapshot", {
  ...
  for (const [index, tool] of tools.entries()) {
    ...
    if (violations.length > 0) {
      _logger.log.warn("google tool schema has unsupported keywords", {
        index,
        tool: tool.name,
...
const compactionFailureEmitter = new _nodeEvents.EventEmitter();
/**
...
(0, _unhandledRejections.registerUnhandledRejectionHandler)((reason) => {
  const message = (0, _utils.describeUnknownError)(reason);
  if (!(0, _piEmbeddedHelpers.isCompactionFailureError)(message)) {
    return false;
  }
  _logger.log.error(`Auto-compaction failed (unhandled): ${message}`);
...
function applyGoogleTurnOrderingFix(params) {
  if (!(0, _piEmbeddedHelpers.isGoogleModelApi)(params.modelApi)) {
  ...
  const sanitized = (0, _piEmbeddedHelpers.sanitizeGoogleTurnOrdering)(params.messages);
  const didPrepend = sanitized !== params.messages;
  if (didPrepend && !hasGoogleTurnOrderingMarker(params.sessionManager)) {
    const warn = params.warn ?? ((message) => _logger.log.warn(message));
...
async function sanitizeSessionHistory(params) {
  ...
  (0, _transcriptPolicy.resolveTranscriptPolicy)({
    modelApi: params.modelApi,
    provider: params.provider,
    modelId: params.modelId
  });
  const sanitizedImages = await (0, _piEmbeddedHelpers.sanitizeSessionMessagesImages)(params.messages, "session:history", {
  ...
  (0, _sessionTranscriptRepair.sanitizeToolUseResultPairing)(sanitizedThinking) :
  sanitizedThinking;
  ...
  (0, _piEmbeddedHelpers.downgradeOpenAIReasoningBlocks)(repairedTools) :
  repairedTools;
