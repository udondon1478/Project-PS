"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.createSlackThreadTsResolver = createSlackThreadTsResolver;var _globals = require("../../globals.js");
...
async function resolveThreadTsFromHistory(params) {
  try {
    ...
    if ((0, _globals.shouldLogVerbose)()) {
      (0, _globals.logVerbose)(`slack inbound: failed to resolve thread_ts via conversations.history for channel=${params.channelId} ts=${params.messageTs}: ${String(err)}`);
    }
    return undefined;
  }
}
function createSlackThreadTsResolver(params) {
  ...
  return {
    resolve: async (request) => {
      ...
      if ((0, _globals.shouldLogVerbose)()) {
        (0, _globals.logVerbose)(`slack inbound: missing thread_ts for thread reply channel=${message.channel} ts=${message.ts} source=${request.source}`);
      }
      ...
      if (resolved) {
        if ((0, _globals.shouldLogVerbose)()) {
          (0, _globals.logVerbose)(`slack inbound: resolved missing thread_ts channel=${message.channel} ts=${message.ts} -> thread_ts=${resolved}`);
        }
        return { ...message, thread_ts: resolved };
      }
      if ((0, _globals.shouldLogVerbose)()) {
        (0, _globals.logVerbose)(`slack inbound: could not resolve missing thread_ts channel=${message.channel} ts=${message.ts}`);
      }
      return message;
    }
  };
