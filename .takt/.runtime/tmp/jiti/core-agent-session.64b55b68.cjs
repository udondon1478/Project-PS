"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.AgentSession = void 0;exports.parseSkillBlock = parseSkillBlock;
...
var _bashExecutor = require("./bash-executor.js");
var _index = require("./compaction/index.js");
var _index2 = require("./export-html/index.js");
var _toolRenderer = require("./export-html/tool-renderer.js");
var _index3 = require("./extensions/index.js");
var _promptTemplates = require("./prompt-templates.js");
var _systemPrompt = require("./system-prompt.js");
var _index4 = require("./tools/index.js"); /**
...
class AgentSession {
  ...
  _rebuildSystemPrompt(toolNames) {
    ...
    return (0, _systemPrompt.buildSystemPrompt)({
      cwd: this._cwd,
      skills: loadedSkills,
  ...
  async prompt(text, options) {
    ...
    if (expandPromptTemplates) {
      ...
      expandedText = (0, _promptTemplates.expandPromptTemplate)(expandedText, [...this.promptTemplates]);
    }
    ...
    if (!this.model) {
      ...
      `Use /login or set an API key environment variable. See ${(0, _nodePath.join)((0, _config.getDocsPath)(), "providers.md")}\n\n` +
  ...
  _expandSkillCommand(text) {
    ...
    try {
      const content = (0, _nodeFs.readFileSync)(skill.filePath, "utf-8");
      const body = (0, _frontmatter.stripFrontmatter)(content).trim();
  ...
  async _emitModelSelect(nextModel, previousModel, source) {
    if (!this._extensionRunner)
    return;
    if ((0, _piAi.modelsAreEqual)(previousModel, nextModel))
    return;
    await this._extensionRunner.emit({
      type: "model_select",
      model: nextModel,
  ...
  async _cycleScopedModel(direction) {
    ...
    let currentIndex = this._scopedModels.findIndex((sm) => (0, _piAi.modelsAreEqual)(sm.model, currentModel));
    if (currentIndex === -1)
    currentIndex = 0;
  ...
  async _cycleAvailableModel(direction) {
    ...
    return undefined;
    const currentModel = this.model;
    let currentIndex = availableModels.findIndex((m) => (0, _piAi.modelsAreEqual)(m, currentModel));
    if (currentIndex === -1)
    currentIndex = 0;
  ...
  supportsXhighThinking() {
    return this.model ? (0, _piAi.supportsXhigh)(this.model) : false;
  }
  /**
  ...
  async compact(customInstructions) {
    ...
    try {
      ...
      const preparation = (0, _index.prepareCompaction)(pathEntries, settings);
      if (!preparation) {
      ...
      } else
      {
        ...
        const result = await (0, _index.compact)(preparation, this.model, apiKey, customInstructions, this._compactionAbortController.signal);
  ...
  async _checkCompaction(assistantMessage, skipAbortedCheck = true) {
    ...
    if (sameModel && !errorIsFromBeforeCompaction && (0, _piAi.isContextOverflow)(assistantMessage, contextWindow)) {
    ...
    return;
    const contextTokens = (0, _index.calculateContextTokens)(assistantMessage.usage);
    if ((0, _index.shouldCompact)(contextTokens, contextWindow, settings)) {
  ...
  async _runAutoCompaction(reason, willRetry) {
    ...
    try {
      ...
      {
        // Generate compaction result
        const compactResult = await (0, _index.compact)(preparation, this.model, apiKey, undefined, this._autoCompactionAbortController.signal);
  ...
  buildExtensionResourcePaths(entries) {
    return entries.map((entry) => {
      ...
      const baseDir = entry.extensionPath.startsWith("<") ? undefined : (0, _nodePath.dirname)(entry.extensionPath);
      return {
        path: entry.path,
        metadata: {
          source,
  ...
  getExtensionSourceLabel(extensionPath) {
    ...
    const base = (0, _nodePath.basename)(extensionPath);
  ...
  _buildRuntime(options) {
    ...
    (0, _index4.createAllTools)(this._cwd, {
    ...
    new _index3.ExtensionRunner(extensionsResult.extensions, extensionsResult.runtime, this._cwd, this.sessionManager, this._modelRegistry) :
    undefined;
    ...
    (0, _index3.wrapRegisteredTools)(allCustomTools, this._extensionRunner) :
    [];
    ...
    if (this._extensionRunner) {
      const wrappedActiveTools = (0, _index3.wrapToolsWithExtensions)(activeToolsArray, this._extensionRunner);
      this.agent.setTools(wrappedActiveTools);
      const wrappedAllTools = (0, _index3.wrapToolsWithExtensions)(Array.from(toolRegistry.values()), this._extensionRunner);
  ...
  _isRetryableError(message) {
    ...
    const contextWindow = this.model?.contextWindow ?? 0;
    if ((0, _piAi.isContextOverflow)(message, contextWindow))
    return false;
  ...
  async _handleRetryableError(message) {
    ...
    try {
      await (0, _sleep.sleep)(delayMs, this._retryAbortController.signal);
    }
    catch {
  ...
  async executeBash(command, onChunk, options) {
    ...
    try {
      ...
      await (0, _bashExecutor.executeBashWithOperations)(resolvedCommand, process.cwd(), options.operations, {
        onChunk,
        signal: this._bashAbortController.signal
      }) :
      await (0, _bashExecutor.executeBash)(resolvedCommand, {
        onChunk,
        signal: this._bashAbortController.signal
      });
  ...
  async navigateTree(targetId, options = {}) {
    ...
    const { entries: entriesToSummarize, commonAncestorId } = (0, _index.collectEntriesForBranchSummary)(this.sessionManager, oldLeafId, targetId);
    ...
    if (options.summarize && entriesToSummarize.length > 0 && !extensionSummary) {
      ...
      const result = await (0, _index.generateBranchSummary)(entriesToSummarize, {
        model,
        apiKey,
        signal: this._branchSummaryAbortController.signal,
        customInstructions,
  ...
  getContextUsage() {
    ...
    if (contextWindow <= 0)
    return undefined;
    const estimate = (0, _index.estimateContextTokens)(this.messages);
  ...
  async exportToHtml(outputPath) {
    ...
    let toolRenderer;
    if (this._extensionRunner) {
      toolRenderer = (0, _toolRenderer.createToolHtmlRenderer)({
        getToolDefinition: (name) => this._extensionRunner.getToolDefinition(name),
        theme: _theme.theme
      });
    }
    return await (0, _index2.exportSessionToHtml)(this.sessionManager, this.state, {
      outputPath,
      themeName,
      toolRenderer
    });
...
}exports.AgentSession = AgentSession; /* v9-ee330b88a70cda41 */
