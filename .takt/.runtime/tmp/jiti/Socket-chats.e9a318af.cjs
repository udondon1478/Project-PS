"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.makeChatsSocket = void 0;var _nodeCache = _interopRequireDefault(require("@cacheable/node-cache"));
...
var _socket = require("./socket.js");function _interopRequireDefault(e) {return e && e.__esModule ? e : { default: e };}
...
const makeChatsSocket = (config) => {
  ...
  const processingMutex = (0, _makeMutex.makeMutex)();
  ...
  const updateProfilePicture = async (jid, content, dimensions) => {
    ...
    const { img } = await (0, _index4.generateProfilePicture)(content, dimensions);
    await query({
      tag: 'iq',
      attrs: {
        to: _index5.S_WHATSAPP_NET,
  ...
  const getBusinessProfile = async (jid) => {
    ...
    if (profiles) {
      ...
      (0, _index5.getBinaryNodeChildren)(businessHours, 'business_hours_config') :
      undefined;
  ...
  const newAppStateChunkHandler = (isInitialSync) => {
    return {
      onMutation(mutation) {
        (0, _index4.processSyncAction)(mutation, ev, authState.creds.me, isInitialSync ? { accountSettings: authState.creds.accountSettings } : undefined, logger);
      }
    };
  };
    ...
    await authState.keys.transaction(async () => {
      ...
      while (collectionsToHandle.size) {
        ...
        for (const key in decoded) {
          ...
          try {
            if (snapshot) {
              const { state: newState, mutationMap } = await (0, _index4.decodeSyncdSnapshot)(name, snapshot, getAppStateSyncKey, initialVersionMap[name], appStateMacVerification.snapshot);
            ...
            if (patches.length) {
              const { state: newState, mutationMap } = await (0, _index4.decodePatches)(name, patches, states[name], getAppStateSyncKey, config.options, initialVersionMap[name], logger, appStateMacVerification.patch);
              await authState.keys.set({ 'app-state-sync-version': { [name]: newState } });
  ...
  const sendPresenceUpdate = async (type, toJid) => {
    ...
    {
      const { server } = (0, _index5.jidDecode)(toJid);
  ...
  const appPatch = async (patchCreate) => {
    ...
    if (!myAppStateKeyId) {
      throw new _boom.Boom('App state key not present!', { statusCode: 400 });
    }
    let initial;
    let encodeResult;
    await processingMutex.mutex(async () => {
      await authState.keys.transaction(async () => {
        ...
        initial = currentSyncVersion || (0, _index4.newLTHashState)();
    ...
    if (config.emitOwnEvents) {
      ...
      const { mutationMap } = await (0, _index4.decodePatches)(name, [{ ...encodeResult.patch, version: { version: encodeResult.state.version } }], initial, getAppStateSyncKey, config.options, undefined, logger);
  ...
  const upsertMessage = ev.createBufferedFunction(async (msg, type) => {
    ...
    _index2.PROCESSABLE_HISTORY_TYPES.includes(historyMsg.syncType) :
    false;
  ...
  ws.on('CB:ib,,dirty', async (node) => {
    const { attrs } = (0, _index5.getBinaryNodeChild)(node, 'dirty');
...
};exports.makeChatsSocket = makeChatsSocket; /* v9-78f668ff34e73ae8 */
