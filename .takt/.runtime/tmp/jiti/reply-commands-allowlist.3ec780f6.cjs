"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.handleAllowlistCommand = void 0;var _dock = require("../../channels/dock.js");
...
var _accounts = require("../../discord/accounts.js");
...
function normalizeAllowFrom(params) {
  const dock = (0, _dock.getChannelDock)(params.channelId);
...
function resolveAccountTarget(parsed, channelId, accountId) {
  ...
  const useAccount = normalizedAccountId !== _sessionKey.DEFAULT_ACCOUNT_ID || hasAccounts;
  if (!useAccount) {
...
async function resolveSlackNames(params) {
  const account = (0, _accounts4.resolveSlackAccount)({ cfg: params.cfg, accountId: params.accountId });
  const token = account.config.userToken?.trim() || account.botToken?.trim();
  if (!token) {
    return new Map();
  }
  const resolved = await (0, _resolveUsers2.resolveSlackUserAllowlist)({ token, entries: params.entries });
  const map = new Map();
...
async function resolveDiscordNames(params) {
  ...
  const resolved = await (0, _resolveUsers.resolveDiscordUserAllowlist)({ token, entries: params.entries });
  const map = new Map();
  for (const entry of resolved) {
    if (entry.resolved && entry.name) {
      map.set(entry.input, entry.name);
...
const handleAllowlistCommand = async (params, allowTextCommands) => {
  ...
  if (!params.command.isAuthorizedSender) {
    (0, _globals.logVerbose)(`Ignoring /allowlist from unauthorized sender: ${params.command.senderId || "<unknown>"}`);
    return { shouldContinue: false };
  }
  const channelId = (0, _registry.normalizeChannelId)(parsed.channel) ??
  params.command.channelId ??
  (0, _registry.normalizeChannelId)(params.command.channel);
  if (!channelId) {
    return {
  ...
  const accountId = (0, _sessionKey.normalizeAccountId)(parsed.account ?? params.ctx.AccountId);
  const scope = parsed.scope;
  if (parsed.action === "list") {
    const pairingChannels = (0, _pairing.listPairingChannels)();
    const supportsStore = pairingChannels.includes(channelId);
    const storeAllowFrom = supportsStore ?
    await (0, _pairingStore.readChannelAllowFromStore)(channelId).catch(() => []) :
    [];
    let dmAllowFrom = [];
    let groupAllowFrom = [];
    let groupOverrides = [];
    ...
    let groupPolicy;
    if (channelId === "telegram") {
      const account = (0, _accounts5.resolveTelegramAccount)({ cfg: params.cfg, accountId });
    ...
    if (channelId === "whatsapp") {
      const account = (0, _accounts6.resolveWhatsAppAccount)({ cfg: params.cfg, accountId });
    ...
    if (channelId === "signal") {
      const account = (0, _accounts3.resolveSignalAccount)({ cfg: params.cfg, accountId });
      dmAllowFrom = (account.config.allowFrom ?? []).map(String);
      groupAllowFrom = (account.config.groupAllowFrom ?? []).map(String);
      dmPolicy = account.config.dmPolicy;
      groupPolicy = account.config.groupPolicy;
    ...
    if (channelId === "imessage") {
      const account = (0, _accounts2.resolveIMessageAccount)({ cfg: params.cfg, accountId });
      dmAllowFrom = (account.config.allowFrom ?? []).map(String);
      groupAllowFrom = (account.config.groupAllowFrom ?? []).map(String);
      dmPolicy = account.config.dmPolicy;
      groupPolicy = account.config.groupPolicy;
    ...
    if (channelId === "slack") {
      const account = (0, _accounts4.resolveSlackAccount)({ cfg: params.cfg, accountId });
    ...
    if (channelId === "discord") {
      const account = (0, _accounts.resolveDiscordAccount)({ cfg: params.cfg, accountId });
  ...
  const shouldTouchStore = parsed.target !== "config" && (0, _pairing.listPairingChannels)().includes(channelId);
  if (shouldUpdateConfig) {
    ...
    if (configChanged) {
      const validated = (0, _config.validateConfigObjectWithPlugins)(parsedConfig);
      if (!validated.ok) {
    ...
    if (shouldTouchStore) {
      if (parsed.action === "add") {
        await (0, _pairingStore.addChannelAllowFromStoreEntry)({ channel: channelId, entry: parsed.entry });
      } else
      if (parsed.action === "remove") {
        await (0, _pairingStore.removeChannelAllowFromStoreEntry)({ channel: channelId, entry: parsed.entry });
      }
    }
...
};exports.handleAllowlistCommand = handleAllowlistCommand; /* v9-55a276102d316a7a */
