"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports._test = exports.TTS_PROVIDERS = exports.OPENAI_TTS_VOICES = exports.OPENAI_TTS_MODELS = void 0;exports.buildTtsSystemPromptHint = buildTtsSystemPromptHint;exports.getLastTtsAttempt = getLastTtsAttempt;exports.getTtsMaxLength = getTtsMaxLength;exports.getTtsProvider = getTtsProvider;exports.isSummarizationEnabled = isSummarizationEnabled;exports.isTtsEnabled = isTtsEnabled;exports.isTtsProviderConfigured = isTtsProviderConfigured;exports.maybeApplyTtsToPayload = maybeApplyTtsToPayload;exports.normalizeTtsAutoMode = normalizeTtsAutoMode;exports.resolveTtsApiKey = resolveTtsApiKey;exports.resolveTtsAutoMode = resolveTtsAutoMode;exports.resolveTtsConfig = resolveTtsConfig;exports.resolveTtsPrefsPath = resolveTtsPrefsPath;exports.resolveTtsProviderOrder = resolveTtsProviderOrder;exports.setLastTtsAttempt = setLastTtsAttempt;exports.setSummarizationEnabled = setSummarizationEnabled;exports.setTtsAutoMode = setTtsAutoMode;exports.setTtsEnabled = setTtsEnabled;exports.setTtsMaxLength = setTtsMaxLength;exports.setTtsProvider = setTtsProvider;exports.textToSpeech = textToSpeech;exports.textToSpeechTelephony = textToSpeechTelephony;var _piAi = require("@mariozechner/pi-ai");
...
var _model = require("../agents/pi-embedded-runner/model.js");
...
function resolveTtsPrefsPath(config) {
  if (config.prefsPath?.trim()) {
    return (0, _utils.resolveUserPath)(config.prefsPath.trim());
  }
  const envPath = process.env.OPENCLAW_TTS_PREFS?.trim();
  if (envPath) {
    return (0, _utils.resolveUserPath)(envPath);
  }
  return _nodePath.default.join(_utils.CONFIG_DIR, "settings", "tts.json");
}
...
function readPrefs(prefsPath) {
  try {
    if (!(0, _nodeFs.existsSync)(prefsPath)) {
      return {};
    }
...
function atomicWriteFileSync(filePath, content) {
  ...
  (0, _nodeFs.writeFileSync)(tmpPath, content);
  try {
...
function resolveChannelId(channel) {
  return channel ? (0, _index.normalizeChannelId)(channel) : null;
}
...
const TTS_PROVIDERS = exports.TTS_PROVIDERS = ["openai", "elevenlabs", "edge"];
...
const OPENAI_TTS_MODELS = exports.OPENAI_TTS_MODELS = ["gpt-4o-mini-tts", "tts-1", "tts-1-hd"];
/**
...
const OPENAI_TTS_VOICES = exports.OPENAI_TTS_VOICES = [
"alloy",
"ash",
"coral",
"echo",
...
function resolveSummaryModelRef(cfg, config) {
  const defaultRef = (0, _modelSelection.resolveDefaultModelForAgent)({ cfg });
  const override = config.summaryModel?.trim();
  if (!override) {
    return { ref: defaultRef, source: "default" };
  }
  const aliasIndex = (0, _modelSelection.buildModelAliasIndex)({ cfg, defaultProvider: defaultRef.provider });
...
async function summarizeText(params) {
  ...
  const resolved = (0, _model.resolveModel)(ref.provider, ref.model, undefined, cfg);
  if (!resolved.model) {
    throw new Error(resolved.error ?? `Unknown summary model: ${ref.provider}/${ref.model}`);
  }
  const apiKey = (0, _modelAuth.requireApiKey)(await (0, _modelAuth.getApiKeyForModel)({ model: resolved.model, cfg }), ref.provider);
  try {
    const controller = new AbortController();
    const timeout = setTimeout(() => controller.abort(), timeoutMs);
    try {
      const res = await (0, _piAi.completeSimple)(resolved.model, {
        messages: [
        {
          role: "user",
  ...
  for (const provider of providers) {
    ...
    try {
      if (provider === "edge") {
        ...
        const tempDir = (0, _nodeFs.mkdtempSync)(_nodePath.default.join((0, _nodeOs.tmpdir)(), "tts-"));
        ...
        const attemptEdgeTts = async (outputFormat) => {
          ...
          const audioPath = _nodePath.default.join(tempDir, `voice-${Date.now()}${extension}`);
          await edgeTTS({
            text: params.text,
        ...
        try {
          ...
          if (fallbackEdgeOutputFormat && fallbackEdgeOutputFormat !== edgeOutputFormat) {
            (0, _globals.logVerbose)(`TTS: Edge output ${edgeOutputFormat} failed; retrying with ${fallbackEdgeOutputFormat}.`);
        ...
        const voiceCompatible = (0, _audio.isVoiceCompatibleAudio)({ fileName: edgeResult.audioPath });
        return {
          success: true,
      ...
      const tempDir = (0, _nodeFs.mkdtempSync)(_nodePath.default.join((0, _nodeOs.tmpdir)(), "tts-"));
      const audioPath = _nodePath.default.join(tempDir, `voice-${Date.now()}${output.extension}`);
      (0, _nodeFs.writeFileSync)(audioPath, audioBuffer);
      scheduleCleanup(tempDir);
      return {
        success: true,
        audioPath,
...
async function maybeApplyTtsToPayload(params) {
  ...
  if (directives.warnings.length > 0) {
    (0, _globals.logVerbose)(`TTS: ignored directive overrides (${directives.warnings.join("; ")})`);
  }
  ...
  if (textForAudio.length > maxLength) {
    if (!isSummarizationEnabled(prefsPath)) {
      ...
      (0, _globals.logVerbose)(`TTS: truncating long text (${textForAudio.length} > ${maxLength}), summarization disabled.`);
    ...
    } else
    {
      ...
      try {
        ...
        if (textForAudio.length > config.maxTextLength) {
          (0, _globals.logVerbose)(`TTS: summary exceeded hard limit (${textForAudio.length} > ${config.maxTextLength}); truncating.`);
          textForAudio = `${textForAudio.slice(0, config.maxTextLength - 3)}...`;
        }
      }
      catch (err) {
        const error = err;
        (0, _globals.logVerbose)(`TTS: summarization failed, truncating instead: ${error.message}`);
        textForAudio = `${textForAudio.slice(0, maxLength - 3)}...`;
      }
    }
  }
  ...
  (0, _globals.logVerbose)(`TTS: conversion failed after ${latency}ms (${result.error ?? "unknown"}).`);
  return nextPayload;
}
