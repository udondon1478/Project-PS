"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.OptionsHandler = void 0;var _v = require("discord-api-types/v10");
 ...
 */
class OptionsHandler extends _Base.Base {
  /**
  ...
  constructor({ client, options, interactionData, definitions, guildId }) {
    ...
    for (const option of options) {
      if (option.type === _v.ApplicationCommandOptionType.Subcommand) {
        for (const subOption of option.options ?? []) {
          this.raw.push(subOption);
        }
      } else
      if (option.type === _v.ApplicationCommandOptionType.SubcommandGroup) {
        for (const subOption of option.options ?? []) {
  ...
  getString(key, required = false) {
    const value = this.raw.find((x) => x.name === key && x.type === _v.ApplicationCommandOptionType.String)?.value;
    if (required) {
  ...
  getInteger(key, required = false) {
    const value = this.raw.find((x) => x.name === key && x.type === _v.ApplicationCommandOptionType.Integer)?.value;
    if (required) {
  ...
  getNumber(key, required = false) {
    const value = this.raw.find((x) => x.name === key && x.type === _v.ApplicationCommandOptionType.Number)?.value;
    if (required) {
  ...
  getBoolean(key, required = false) {
    const value = this.raw.find((x) => x.name === key && x.type === _v.ApplicationCommandOptionType.Boolean)?.value;
    if (required) {
  ...
  getUser(key, required = false) {
    const id = this.raw.find((x) => x.name === key && x.type === _v.ApplicationCommandOptionType.User)?.value;
    if (required) {
    ...
    return new _index.User(this.client, user);
  }
  getMember(key, required = false) {
    ...
    member = new _index.GuildMember(this.client, { ...rawMember, mute: undefined, deaf: undefined, user }, new _index.Guild(this.client, guildId));
    return member;
  }
  async getChannelId(key, required = false) {
    const id = this.raw.find((x) => x.name === key && x.type === _v.ApplicationCommandOptionType.Channel)?.value;
    if (required) {
      if (!id || typeof id !== "string")
      throw new Error(`Missing required option: ${key}`);
    } else
  ...
  getRole(key, required = false) {
    const id = this.raw.find((x) => x.name === key && x.type === _v.ApplicationCommandOptionType.Role)?.value;
    if (required) {
      if (!id || typeof id !== "string")
      throw new Error(`Missing required option: ${key}`);
    } else
    ...
    return new _index.Role(this.client, role, this.guildId);
  }
  getMentionable(key, required = false) {
    const id = this.raw.find((x) => x.name === key && x.type === _v.ApplicationCommandOptionType.Mentionable)?.value;
    if (required) {
      if (!id || typeof id !== "string")
      throw new Error(`Missing required option: ${key}`);
    } else
  ...
  getAttachment(key, required = false) {
    ...
    const id = this.raw.find((x) => x.name === key && x.type === _v.ApplicationCommandOptionType.Attachment)?.value;
    if (required) {
      if (!id || typeof id !== "string")
      throw new Error(`Missing required option: ${key}`);
    } else
  ...
  checkAgainstDefinition(key, value) {
    ...
    if (!definition)
    return;
    if (definition.type === _v.ApplicationCommandOptionType.String &&
    typeof value === "string") {
    ...
    if ((definition.type === _v.ApplicationCommandOptionType.Integer ||
    definition.type === _v.ApplicationCommandOptionType.Number) &&
    typeof value === "number") {
...
}exports.OptionsHandler = OptionsHandler; /* v9-5f2fd0ede4bd84d1 */
