"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.convertMessages = convertMessages;exports.streamSimpleOpenAICompletions = exports.streamOpenAICompletions = void 0;var _openai = _interopRequireDefault(require("openai"));
...
var _transformMessages = require("./transform-messages.js");function _interopRequireDefault(e) {return e && e.__esModule ? e : { default: e };}
/**
...
const streamOpenAICompletions = (model, context, options) => {
            ...
            for (const toolCall of choice.delta.tool_calls) {
              ...
              if (currentBlock.type === "toolCall") {
                ...
                if (toolCall.function?.arguments) {
                  ...
                  currentBlock.arguments = (0, _jsonParse.parseStreamingJson)(currentBlock.partialArgs);
                }
                stream.push({
                  type: "toolcall_delta",
                  contentIndex: blockIndex(),
  ...
  return stream;
};exports.streamOpenAICompletions = streamOpenAICompletions;
...
};exports.streamSimpleOpenAICompletions = streamSimpleOpenAICompletions;
...
function convertMessages(model, context, compat) {
  ...
  const transformedMessages = (0, _transformMessages.transformMessages)(context.messages, model, (id) => normalizeToolCallId(id));
  if (context.systemPrompt) {
    const useDeveloperRole = model.reasoning && compat.supportsDeveloperRole;
    const role = useDeveloperRole ? "developer" : "system";
    params.push({ role: role, content: (0, _sanitizeUnicode.sanitizeSurrogates)(context.systemPrompt) });
  }
  let lastRole = null;
  for (let i = 0; i < transformedMessages.length; i++) {
    ...
    if (msg.role === "user") {
      if (typeof msg.content === "string") {
        params.push({
          role: "user",
          content: (0, _sanitizeUnicode.sanitizeSurrogates)(msg.content)
        });
      } else
      {
        const content = msg.content.map((item) => {
          if (item.type === "text") {
            return {
              type: "text",
              text: (0, _sanitizeUnicode.sanitizeSurrogates)(item.text)
            };
          } else
          {
            return {
    ...
    if (msg.role === "assistant") {
      ...
      if (nonEmptyTextBlocks.length > 0) {
        ...
        if (model.provider === "github-copilot") {
          assistantMsg.content = nonEmptyTextBlocks.map((b) => (0, _sanitizeUnicode.sanitizeSurrogates)(b.text)).join("");
        } else
        {
          assistantMsg.content = nonEmptyTextBlocks.map((b) => {
            return { type: "text", text: (0, _sanitizeUnicode.sanitizeSurrogates)(b.text) };
          });
        }
      }
    ...
    if (msg.role === "toolResult") {
      ...
      for (; j < transformedMessages.length && transformedMessages[j].role === "toolResult"; j++) {
        ...
        const toolResultMsg = {
          role: "tool",
          content: (0, _sanitizeUnicode.sanitizeSurrogates)(hasText ? textResult : "(see attached image)"),
