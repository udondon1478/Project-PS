"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.encryptedStream = exports.encryptMediaRetryRequest = exports.encodeBase64EncodedStringForUpload = exports.downloadEncryptedContent = exports.downloadContentFromMessage = exports.decryptMediaRetryData = exports.decodeMediaRetryNode = void 0;exports.extensionForMediaMessage = extensionForMediaMessage;exports.generateProfilePicture = exports.extractImageThumb = void 0;exports.generateThumbnail = generateThumbnail;exports.getAudioDuration = getAudioDuration;exports.getAudioWaveform = getAudioWaveform;exports.getHttpStream = void 0;exports.getMediaKeys = getMediaKeys;exports.toReadable = exports.toBuffer = exports.mediaMessageSHA256B64 = exports.hkdfInfoKey = exports.getWAUploadToServer = exports.getUrlFromDirectPath = exports.getStream = exports.getStatusCodeForMediaRetry = exports.getRawMediaUploadData = void 0;var _boom = require("@hapi/boom");
...
var _generics = require("./generics.js");function _interopRequireWildcard(e, t) {if ("function" == typeof WeakMap) var r = new WeakMap(),n = new WeakMap();return (_interopRequireWildcard = function (e, t) {if (!t && e && e.__esModule) return e;var o,i,f = { __proto__: null, default: e };if (null === e || "object" != typeof e && "function" != typeof e) return f;if (o = t ? n : r) {if (o.has(e)) return o.get(e);o.set(e, f);}for (const t in e) "default" !== t && {}.hasOwnProperty.call(e, t) && ((i = (o = Object.defineProperty) && Object.getOwnPropertyDescriptor(e, t)) && (i.get || i.set) ? o(f, t, i) : f[t] = e[t]);return f;})(e, t);}
const getTmpFilesDirectory = () => (0, _os.tmpdir)();
const getImageProcessingLibrary = async () => {
  //@ts-ignore
  const [jimp, sharp] = await Promise.all([Promise.resolve().then(() => jitiImport('jimp').then((m) => _interopRequireWildcard(m))).catch(() => {}), Promise.resolve().then(() => jitiImport('sharp').then((m) => _interopRequireWildcard(m))).catch(() => {})]);
  if (sharp) {
    return { sharp };
  }
  if (jimp) {
...
/** generates all the keys required to encrypt/decrypt & sign a media message */exports.getRawMediaUploadData = getRawMediaUploadData;
async function getMediaKeys(buffer, mediaType) {
  ...
  const expandedMediaKey = await (0, _crypto2.hkdf)(buffer, 112, { info: hkdfInfoKey(mediaType) });
  return {
...
const encodeBase64EncodedStringForUpload = (b64) => encodeURIComponent(b64.replace(/\+/g, '-').replace(/\//g, '_').replace(/\=+$/, ''));exports.encodeBase64EncodedStringForUpload = encodeBase64EncodedStringForUpload;
...
/** gets the SHA256 of the given media message */exports.generateProfilePicture = generateProfilePicture;
const mediaMessageSHA256B64 = (message) => {
  const media = Object.values(message)[0];
  return media?.fileSha256 && Buffer.from(media.fileSha256).toString('base64');
};exports.mediaMessageSHA256B64 = mediaMessageSHA256B64;
async function getAudioDuration(buffer) {
  const musicMetadata = await Promise.resolve().then(() => jitiImport('music-metadata').then((m) => _interopRequireWildcard(m)));
  let metadata;
  const options = {
    duration: true
  };
...
async function getAudioWaveform(buffer, logger) {
  try {
    // @ts-ignore
    const { default: decoder } = await Promise.resolve().then(() => jitiImport('audio-decode').then((m) => _interopRequireWildcard(m)));
    let audioData;
    if (Buffer.isBuffer(buffer)) {
      audioData = buffer;
    } else
...
const toReadable = (buffer) => {
  const readable = new _stream.Readable({ read: () => {} });
...
const getStream = async (item, opts) => {
  ...
  return { stream: (0, _fs.createReadStream)(item.url), type: 'file' };
};
/** generates a thumbnail for a given media, if required */exports.getStream = getStream;
...
const getHttpStream = async (url, options = {}) => {
  ...
  if (!response.ok) {
    throw new _boom.Boom(`Failed to fetch stream from ${url}`, { statusCode: response.status, data: { url } });
  }
...
const getUrlFromDirectPath = (directPath) => `https://${DEF_HOST}${directPath}`;exports.getUrlFromDirectPath = getUrlFromDirectPath;
const downloadContentFromMessage = async ({ mediaKey, directPath, url }, type, opts = {}) => {
  const isValidMediaUrl = url?.startsWith('https://mmg.whatsapp.net/');
  const downloadUrl = isValidMediaUrl ? url : getUrlFromDirectPath(directPath);
  if (!downloadUrl) {
    throw new _boom.Boom('No valid media URL or directPath present in message', { statusCode: 400 });
  }
...
const getWAUploadToServer = ({ customUploadHosts, fetchAgent, logger, options }, refreshMediaConn) => {
  return async (filePath, { mediaType, fileEncSha256B64, timeoutMs }) => {
    ...
    if (!urls) {
      throw new _boom.Boom('Media upload failed on all hosts', { statusCode: 500 });
    }
    return urls;
  };
};exports.getWAUploadToServer = getWAUploadToServer;
const getMediaRetryKey = (mediaKey) => {
  return (0, _crypto2.hkdf)(mediaKey, 32, { info: 'WhatsApp Media Retry Notification' });
};
/**
...
const decodeMediaRetryNode = (node) => {
  ...
  } else
  {
    ...
    {
      event.error = new _boom.Boom('Failed to re-upload media (missing ciphertext)', { statusCode: 404 });
    }
  }
  return event;
...
const getStatusCodeForMediaRetry = (code) => MEDIA_RETRY_STATUS_MAP[code];exports.getStatusCodeForMediaRetry = getStatusCodeForMediaRetry;
