"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.OSC_PROGRESS_ST = exports.OSC_PROGRESS_PREFIX = exports.OSC_PROGRESS_C1_ST = exports.OSC_PROGRESS_BEL = void 0;exports.createOscProgressController = createOscProgressController;exports.findOscProgressSequences = findOscProgressSequences;exports.sanitizeLabel = sanitizeLabel;exports.sanitizeOscProgress = sanitizeOscProgress;exports.startOscProgress = startOscProgress;exports.stripOscProgress = stripOscProgress;exports.supportsOscProgress = supportsOscProgress;var _nodeProcess = _interopRequireDefault(require("node:process"));function _interopRequireDefault(e) {return e && e.__esModule ? e : { default: e };}
/**
 ...
 */
const OSC_PROGRESS_PREFIX = exports.OSC_PROGRESS_PREFIX = '\u001b]9;4;';
/** String Terminator (ST): `ESC \\` */
const OSC_PROGRESS_ST = exports.OSC_PROGRESS_ST = '\u001b\\';
/** Bell (BEL): `0x07` */
const OSC_PROGRESS_BEL = exports.OSC_PROGRESS_BEL = '\u0007';
/** C1 String Terminator (ST): `0x9c` */
const OSC_PROGRESS_C1_ST = exports.OSC_PROGRESS_C1_ST = '\u009c';
 ...
 */
function supportsOscProgress(env = _nodeProcess.default.env, isTty = _nodeProcess.default.stdout.isTTY, options = {}) {
  if (!isTty)
  return false;
  if (options.disabled)
  return false;
 ...
 */
function findOscProgressSequences(text) {
  const sequences = [];
 ...
 */
function stripOscProgress(text) {
  const prefixLen = OSC_PROGRESS_PREFIX.length;
  let current = text;
 ...
 */
function sanitizeOscProgress(text, keepOsc) {
...
function startOscProgress(options = {}) {
  const { label = 'Working…', targetMs = 10 * 60_000, write = (text) => _nodeProcess.default.stderr.write(text), indeterminate = false, state = 1, terminator } = options;
...
function createOscProgressController(options = {}) {
  ...
  const { label = 'Working…', write = (text) => _nodeProcess.default.stderr.write(text), terminator, stallAfterMs = 0, stalledLabel, clearDelayMs = DEFAULT_CLEAR_DELAY_MS, autoClearOnExit = false } = options;
  if (!supportsOscProgress(options.env, options.isTty, options)) {
  ...
  if (autoClearOnExit) {
    _nodeProcess.default.once('exit', handleExit);
  }
  const dispose = () => {
    clearTimers();
    if (autoClearOnExit) {
      _nodeProcess.default.off('exit', handleExit);
    }
  };
