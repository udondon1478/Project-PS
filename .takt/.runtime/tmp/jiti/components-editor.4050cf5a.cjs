"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.Editor = void 0;exports.wordWrapLine = wordWrapLine;var _keybindings = require("../keybindings.js");
...
function wordWrapLine(line, maxWidth) {
  ...
  const lineWidth = (0, _utils.visibleWidth)(line);
  ...
  for (let i = 0; i < segments.length; i++) {
    ...
    const gWidth = (0, _utils.visibleWidth)(grapheme);
    const charIndex = seg.index;
    const isWs = (0, _utils.isWhitespaceChar)(grapheme);
    ...
    if (isWs && next && !(0, _utils.isWhitespaceChar)(next.segment)) {
...
class Editor {
  ...
  invalidate() {
    ...
    if (cursorLineIndex >= this.scrollOffset + maxVisibleLines) {
    ...
    if (this.scrollOffset > 0) {
      ...
      const remaining = width - (0, _utils.visibleWidth)(indicator);
    ...
    for (const layoutLine of visibleLines) {
      ...
      let lineVisibleWidth = (0, _utils.visibleWidth)(layoutLine.text);
      ...
      if (layoutLine.hasCursor && layoutLine.cursorPos !== undefined) {
        ...
        const marker = emitCursorMarker ? _tui.CURSOR_MARKER : "";
        if (after.length > 0) {
  ...
  handleInput(data) {
    ...
    if (kb.matches(data, "deleteCharBackward") || (0, _keys.matchesKey)(data, "shift+backspace")) {
      this.handleBackspace();
      return;
    }
    if (kb.matches(data, "deleteCharForward") || (0, _keys.matchesKey)(data, "shift+delete")) {
    ...
    if (kb.matches(data, "cursorUp")) {
      ...
      if (this.historyIndex > -1 && this.isOnFirstVisualLine()) {
        this.navigateHistory(-1);
      } else
    ...
    if ((0, _keys.matchesKey)(data, "shift+space")) {
  ...
  layoutText(contentWidth) {
    ...
    for (let i = 0; i < this.state.lines.length; i++) {
      ...
      const lineVisibleWidth = (0, _utils.visibleWidth)(line);
  ...
  insertCharacter(char, skipUndoCoalescing) {
    ...
    if (!skipUndoCoalescing) {
      if ((0, _utils.isWhitespaceChar)(char) || this.lastAction !== "type-word") {
        this.pushUndoSnapshot();
      }
  ...
  buildVisualLineMap(width) {
    ...
    for (let i = 0; i < this.state.lines.length; i++) {
      const line = this.state.lines[i] || "";
      const lineVisWidth = (0, _utils.visibleWidth)(line);
      if (line.length === 0) {
  ...
  tryTriggerAutocomplete(explicitTab = false) {
    ...
    if (suggestions && suggestions.items.length > 0) {
      ...
      this.autocompleteList = new _selectList.SelectList(suggestions.items, this.autocompleteMaxVisible, this.theme.selectList);
...
}exports.Editor = Editor; /* v9-d964c0fe12aff0f5 */
