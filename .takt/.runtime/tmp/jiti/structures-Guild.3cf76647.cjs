"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.Guild = void 0;var _v = require("discord-api-types/v10");
var _Base = require("../abstracts/Base.js");
var _DiscordError = require("../errors/DiscordError.js");
var _channelFactory = require("../functions/channelFactory.js");
var _index = require("../utils/index.js");
var _Emoji = require("./Emoji.js");
var _GuildMember = require("./GuildMember.js");
var _GuildScheduledEvent = require("./GuildScheduledEvent.js");
var _Role = require("./Role.js");
class Guild extends _Base.Base {
  ...
  get iconUrl() {
    if (!this._rawData)
    return undefined;
    return (0, _index.buildCDNUrl)(`https://cdn.discordapp.com/icons/${this.id}`, this.icon);
  }
  /**
  ...
  getIconUrl(options) {
    if (!this._rawData)
    return undefined;
    return (0, _index.buildCDNUrl)(`https://cdn.discordapp.com/icons/${this.id}`, this.icon, options);
  }
  /**
  ...
  get splashUrl() {
    if (!this._rawData)
    return undefined;
    return (0, _index.buildCDNUrl)(`https://cdn.discordapp.com/splashes/${this.id}`, this.splash);
  }
  /**
  ...
  getSplashUrl(options) {
    if (!this._rawData)
    return undefined;
    return (0, _index.buildCDNUrl)(`https://cdn.discordapp.com/splashes/${this.id}`, this.splash, options);
  }
  /**
  ...
  get roles() {
    ...
    return roles.map((role) => new _Role.Role(this.client, role, this.id));
  }
  /**
  ...
  get discoverySplashUrl() {
    if (!this._rawData)
    return undefined;
    return (0, _index.buildCDNUrl)(`https://cdn.discordapp.com/discovery-splashes/${this.id}`, this.discoverySplash);
  }
  /**
  ...
  getDiscoverySplashUrl(options) {
    if (!this._rawData)
    return undefined;
    return (0, _index.buildCDNUrl)(`https://cdn.discordapp.com/discovery-splashes/${this.id}`, this.discoverySplash, options);
  }
  /**
  ...
  get emojis() {
    if (!this._rawData)
    return undefined;
    return this._rawData.emojis.map((emoji) => new _Emoji.GuildEmoji(this.client, emoji, this.id));
  }
  /**
  ...
  get bannerUrl() {
    if (!this._rawData)
    return undefined;
    return (0, _index.buildCDNUrl)(`https://cdn.discordapp.com/banners/${this.id}`, this.banner);
  }
  /**
  ...
  getBannerUrl(options) {
    if (!this._rawData)
    return undefined;
    return (0, _index.buildCDNUrl)(`https://cdn.discordapp.com/banners/${this.id}`, this.banner, options);
  }
  /**
   ...
   */
  async fetch() {
    const newData = await this.client.rest.get(_v.Routes.guild(this.id));
    if (!newData)
   ...
   */
  async leave() {
    await this.client.rest.delete(_v.Routes.guild(this.id));
  }
  /**
   * Create a role in the guild
   */
  async createRole(data) {
    const role = await this.client.rest.post(_v.Routes.guildRoles(this.id), {
      body: {
        ...data
      }
    });
    const roleClass = new _Role.Role(this.client, role, this.id);
  ...
  async fetchMember(memberId) {
    try {
      ...
      const member = await this.client.rest.get(_v.Routes.guildMember(this.id, memberId));
      const memberObject = new _GuildMember.GuildMember(this.client, member, partialGuild);
      return memberObject;
    }
    catch (e) {
      if (e instanceof _DiscordError.DiscordError) {
        if (e.status === 404)
        return null;
      }
      throw e;
  ...
  async fetchMembers(limit = 100) {
    if (limit === "all") {
      ...
      let hasMore = true;
      while (hasMore) {
        const newMembers = await this.client.rest.get(_v.Routes.guildMembers(this.id), {
          limit: "1000",
          ...(after ? { after } : {})
        });
      ...
      const memberObjects = members.map((member) => new _GuildMember.GuildMember(this.client, member, this));
      return memberObjects;
    }
    const cappedLimit = Math.min(limit, 1000);
    const members = await this.client.rest.get(_v.Routes.guildMembers(this.id), {
  ...
  async fetchChannel(channelId) {
    try {
      const channel = await this.client.rest.get(_v.Routes.channel(channelId));
      return (0, _channelFactory.channelFactory)(this.client, channel);
    }
    catch (e) {
      if (e instanceof _DiscordError.DiscordError) {
        if (e.status === 404)
   ...
   */
  async fetchChannels() {
    const channels = await this.client.rest.get(_v.Routes.guildChannels(this.id));
    const channelObjects = channels.map((channel) => (0, _channelFactory.channelFactory)(this.client, channel));
  ...
  async fetchRole(roleId) {
    const role = await this.client.rest.get(_v.Routes.guildRole(this.id, roleId));
    return new _Role.Role(this.client, role, this.id);
  }
  /**
   ...
   */
  async fetchRoles() {
    const roles = await this.client.rest.get(_v.Routes.guildRoles(this.id));
    const roleObjects = roles.map((role) => new _Role.Role(this.client, role, this.id));
    return roleObjects;
  }
  async getEmoji(id) {
    const emoji = await this.client.rest.get(_v.Routes.guildEmoji(this.id, id));
    return new _Emoji.GuildEmoji(this.client, emoji, this.id);
  }
  getEmojiByName(name) {
  ...
  async createEmoji(name, image) {
    const emoji = await this.client.rest.post(_v.Routes.guildEmojis(this.id), {
      body: { name, image }
    });
    return new _Emoji.GuildEmoji(this.client, emoji, this.id);
  }
  async deleteEmoji(id) {
    await this.client.rest.delete(_v.Routes.guildEmoji(this.id, id));
  }
  /**
  ...
  async fetchScheduledEvents(withUserCount = false) {
    const scheduledEvents = await this.client.rest.get(_v.Routes.guildScheduledEvents(this.id), withUserCount ? { with_user_count: "true" } : undefined);
    return scheduledEvents.map((event) => new _GuildScheduledEvent.GuildScheduledEvent(this.client, event, this.id));
  }
  /**
  ...
  async fetchScheduledEvent(eventId, withUserCount = false) {
    try {
      const scheduledEvent = await this.client.rest.get(_v.Routes.guildScheduledEvent(this.id, eventId), withUserCount ? { with_user_count: "true" } : undefined);
      return new _GuildScheduledEvent.GuildScheduledEvent(this.client, scheduledEvent, this.id);
    }
    catch (e) {
      if (e instanceof _DiscordError.DiscordError) {
        if (e.status === 404)
  ...
  async createScheduledEvent(data) {
    const scheduledEvent = await this.client.rest.post(_v.Routes.guildScheduledEvents(this.id), {
      body: {
        name: data.name,
  ...
  async editScheduledEvent(eventId, data) {
    ...
    const scheduledEvent = await this.client.rest.patch(_v.Routes.guildScheduledEvent(this.id, eventId), { body });
    return new _GuildScheduledEvent.GuildScheduledEvent(this.client, scheduledEvent, this.id);
  }
  /**
  ...
  async deleteScheduledEvent(eventId) {
    await this.client.rest.delete(_v.Routes.guildScheduledEvent(this.id, eventId));
  }
  /**
  ...
  async fetchRoleMemberCounts() {
    ...
    return Object.entries(memberCounts).map(([roleId, count]) => ({
      id: roleId,
      role: new _Role.Role(this.client, roleId, this.id),
      count
    }));
  }
}exports.Guild = Guild; /* v9-131dbccd2d665aa6 */
