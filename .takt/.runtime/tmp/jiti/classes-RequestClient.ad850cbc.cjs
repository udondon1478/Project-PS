"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.RequestClient = void 0;var _DiscordError = require("../errors/DiscordError.js");
var _RatelimitError = require("../errors/RatelimitError.js");
...
class RequestClient {
  ...
  async executeRequest(request) {
    ...
    if (response.status === 429) {
      ...
      const rateLimitError = new _RatelimitError.RateLimitError(response, rateLimitBody);
    ...
    if (response.status >= 400 && response.status < 600) {
      ...
      throw new _DiscordError.DiscordError(response, discordErrorBody);
    }
  ...
  async processQueue() {
    ...
    while (this.queue.length > 0) {
      ...
      try {
        ...
        if (error instanceof _RatelimitError.RateLimitError && this.options.queueRequests) {
...
const sleep = (ms) => new Promise((resolve) => setTimeout(resolve, Math.max(ms, 0))); /* v9-cd372c22aaee2aa9 */
