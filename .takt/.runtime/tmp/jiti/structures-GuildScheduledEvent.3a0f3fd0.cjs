"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.GuildScheduledEvent = void 0;var _v = require("discord-api-types/v10");
var _Base = require("../abstracts/Base.js");
var _Guild = require("./Guild.js");
var _User = require("./User.js");
class GuildScheduledEvent extends _Base.Base {
  ...
  get creator() {
    ...
    new _User.User(this.client, this._rawData.creator) :
    null;
  }
  /**
   ...
   */
  async fetch() {
    const newData = await this.client.rest.get(_v.Routes.guildScheduledEvent(this.guildId, this.id));
    if (!newData)
  ...
  async edit(data) {
    ...
    const updatedData = await this.client.rest.patch(_v.Routes.guildScheduledEvent(this.guildId, this.id), { body });
   ...
   */
  async delete() {
    await this.client.rest.delete(_v.Routes.guildScheduledEvent(this.guildId, this.id));
  }
  /**
   * Get the guild this scheduled event belongs to
   */
  async getGuild() {
    const guild = new _Guild.Guild(this.client, this.guildId);
  ...
  async fetchUsers(options) {
    ...
    const result = await this.client.rest.get(_v.Routes.guildScheduledEventUsers(this.guildId, this.id), queryParams);
    return result.map((userData) => new _User.User(this.client, userData.user));
  }
}exports.GuildScheduledEvent = GuildScheduledEvent; /* v9-cb039cb733ddf09d */
