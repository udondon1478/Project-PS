"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.buildInboundMediaNote = buildInboundMediaNote;function formatMediaAttachedLine(params) {
  const prefix = typeof params.index === "number" && typeof params.total === "number" ?
...
function buildInboundMediaNote(ctx) {
  ...
  const urls = Array.isArray(ctx.MediaUrls) && ctx.MediaUrls.length === paths.length ?
  ctx.MediaUrls :
  undefined;
  const types = Array.isArray(ctx.MediaTypes) && ctx.MediaTypes.length === paths.length ?
  ctx.MediaTypes :
  undefined;
  const entries = paths.
  map((entry, index) => ({
