"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.Webhook = void 0;var _v = require("discord-api-types/v10");
var _RequestClient = require("../classes/RequestClient.js");
...
class Webhook {
  ...
  constructor(input) {
    ...
    this.rest = new _RequestClient.RequestClient("webhook");
  }
  _rawData = null;
  setData(data) {
    if (!data)
  ...
  get avatarUrl() {
    if (!this._rawData)
    return undefined;
    return (0, _index.buildCDNUrl)(`https://cdn.discordapp.com/avatars/${this.id}`, this.avatar);
  }
  /**
  ...
  getAvatarUrl(options) {
    if (!this._rawData)
    return undefined;
    return (0, _index.buildCDNUrl)(`https://cdn.discordapp.com/avatars/${this.id}`, this.avatar, options);
  }
  /**
   ...
   */
  async fetch() {
    const newData = await this.rest.get(_v.Routes.webhook(this.id));
    if (!newData)
   ...
   */
  async modify(data) {
    const newData = await this.rest.patch(_v.Routes.webhook(this.id), {
      body: data
    });
    this.setData(newData);
    return this;
   ...
   */
  async delete() {
    await this.rest.delete(_v.Routes.webhook(this.id));
  }
  /**
  ...
  async edit(messageId, data, threadId) {
    ...
    const serialized = (0, _index.serializePayload)(data);
    const finalThreadId = threadId || this.threadId;
    const message = await this.rest.patch(_v.Routes.webhookMessage(this.id, this.token, messageId) + (
  ...
  async deleteMessage(messageId, threadId) {
    ...
    const finalThreadId = threadId || this.threadId;
    await this.rest.delete(_v.Routes.webhookMessage(this.id, this.token, messageId) + (
  ...
  async getMessage(messageId, threadId) {
    ...
    const finalThreadId = threadId || this.threadId;
    const message = await this.rest.get(_v.Routes.webhookMessage(this.id, this.token, messageId) + (
    threadId ? `?thread_id=${threadId}` : ""), finalThreadId ? { thread_id: finalThreadId } : undefined);
    return message;
  }
}exports.Webhook = Webhook; /* v9-dbd8d684fef19069 */
