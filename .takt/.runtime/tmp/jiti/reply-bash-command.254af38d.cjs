"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.handleBashChatCommand = handleBashChatCommand;exports.resetBashChatCommandForTests = resetBashChatCommandForTests;var _agentScope = require("../../agents/agent-scope.js");
var _bashProcessRegistry = require("../../agents/bash-process-registry.js");
var _bashTools = require("../../agents/bash-tools.js");
var _sandbox = require("../../agents/sandbox.js");
var _shellUtils = require("../../agents/shell-utils.js");
...
function resolveForegroundMs(cfg) {
  ...
  return (0, _utils.clampInt)(raw, 0, MAX_FOREGROUND_MS);
}
...
function resolveRawCommandBody(params) {
  ...
  const stripped = (0, _mentions.stripStructuralPrefixes)(source);
  return params.isGroup ?
  (0, _mentions.stripMentions)(stripped, params.ctx, params.cfg, params.agentId) :
  stripped;
}
function getScopedSession(sessionId) {
  const running = (0, _bashProcessRegistry.getSession)(sessionId);
  if (running && running.scopeKey === CHAT_BASH_SCOPE_KEY) {
    return { running };
  }
  const finished = (0, _bashProcessRegistry.getFinishedSession)(sessionId);
...
async function handleBashChatCommand(params) {
  ...
  (0, _agentScope.resolveSessionAgentId)({
  ...
  if (request.action === "stop") {
    ...
    (0, _bashProcessRegistry.markExited)(running, null, "SIGKILL", "failed");
    if (activeJob?.state === "running" && activeJob.sessionId === sessionId) {
      activeJob = null;
    }
    return {
  ...
  try {
    ...
    if (result.details?.status === "running") {
      ...
      (0, _globals.logVerbose)(`Started bash session ${snippet}: ${commandText}`);
      return {
