"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.CombinedAutocompleteProvider = void 0;var _child_process = require("child_process");
var _fs = require("fs");
var _os = require("os");
var _path = require("path");
var _fuzzy = require("./fuzzy.js");
...
class CombinedAutocompleteProvider {
  ...
  getSuggestions(lines, cursorLine, cursorCol) {
    ...
    if (textBeforeCursor.startsWith("/")) {
      ...
      if (spaceIndex === -1) {
        ...
        const filtered = (0, _fuzzy.fuzzyFilter)(commandItems, prefix, (item) => item.name).map((item) => ({
          value: item.name,
          label: item.label,
  ...
  expandHomePath(path) {
    if (path.startsWith("~/")) {
      const expandedPath = (0, _path.join)((0, _os.homedir)(), path.slice(2));
  ...
  getFileSuggestions(prefix) {
    try {
      ...
      if (isRootPrefix) {
        ...
        {
          searchDir = (0, _path.join)(this.basePath, expandedPrefix);
        }
        searchPrefix = "";
      } else
      ...
      } else
      {
        ...
        const dir = (0, _path.dirname)(expandedPrefix);
        const file = (0, _path.basename)(expandedPrefix);
        if (rawPrefix.startsWith("~") || expandedPrefix.startsWith("/")) {
          searchDir = dir;
        } else
        {
          searchDir = (0, _path.join)(this.basePath, dir);
        }
        searchPrefix = file;
      }
      const entries = (0, _fs.readdirSync)(searchDir, { withFileTypes: true });
      const suggestions = [];
      for (const entry of entries) {
        ...
        if (!isDirectory && entry.isSymbolicLink()) {
          try {
            const fullPath = (0, _path.join)(searchDir, entry.name);
            isDirectory = (0, _fs.statSync)(fullPath).isDirectory();
          }
          catch {
        ...
        if (displayPrefix.includes("/")) {
          ...
          if (displayPrefix.startsWith("~/")) {
            ...
            const dir = (0, _path.dirname)(homeRelativeDir);
            relativePath = `~/${dir === "." ? name : (0, _path.join)(dir, name)}`;
          } else
          if (displayPrefix.startsWith("/")) {
            // Absolute path - construct properly
            const dir = (0, _path.dirname)(displayPrefix);
            if (dir === "/") {
              relativePath = `/${name}`;
            } else
            {
          ...
          {
            relativePath = (0, _path.join)((0, _path.dirname)(displayPrefix), name);
          }
        } else
        {
  ...
  getFuzzyFileSuggestions(query, options) {
    ...
    try {
      ...
      for (const { path: entryPath, isDirectory } of topEntries) {
        ...
        const entryName = (0, _path.basename)(pathWithoutSlash);
...
}exports.CombinedAutocompleteProvider = CombinedAutocompleteProvider; /* v9-058da8c803626eea */
