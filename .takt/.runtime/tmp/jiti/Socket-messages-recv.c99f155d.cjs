"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.makeMessagesRecvSocket = void 0;var _nodeCache = _interopRequireDefault(require("@cacheable/node-cache"));
...
var _messagesSend = require("./messages-send.js");function _interopRequireDefault(e) {return e && e.__esModule ? e : { default: e };}
const makeMessagesRecvSocket = (config) => {
  ...
  new _nodeCache.default({
    stdTTL: _index2.DEFAULT_CACHE_TTLS.MSG_RETRY, // 1 hour
    useClones: false
  });
  const callOfferCache = config.callOfferCache ||
  new _nodeCache.default({
    stdTTL: _index2.DEFAULT_CACHE_TTLS.CALL_OFFER, // 5 mins
    useClones: false
  });
  ...
  const identityAssertDebounce = new _nodeCache.default({ stdTTL: 5, useClones: false });
  ...
  const handleNewsletterNotification = async (node) => {
    ...
    const child = (0, _index5.getAllBinaryNodeChildren)(node)[0];
  ...
  const sendMessageAck = async ({ tag, attrs, content }, errorCode) => {
    ...
    if (!!attrs.type && (
    tag !== 'message' || (0, _index5.getBinaryNodeChild)({ tag, attrs, content }, 'unavailable') || errorCode !== 0)) {
      stanza.attrs.type = attrs.type;
    }
    if (tag === 'message' && (0, _index5.getBinaryNodeChild)({ tag, attrs, content }, 'unavailable')) {
  ...
  const sendRetryRequest = async (node, forceIncludeKeys = false) => {
    const { fullMessage } = (0, _index4.decodeMessageNode)(node, authState.creds.me.id, authState.creds.me.lid || '');
    ...
    await authState.keys.transaction(async () => {
      ...
      if (retryCount > 1 || forceIncludeKeys || shouldRecreateSession) {
        const { update, preKeys } = await (0, _index4.getNextPreKeys)(authState, 1);
        ...
        content.push({
          ...
          attrs: {},
          content: [
          { tag: 'type', attrs: {}, content: Buffer.from(_index2.KEY_BUNDLE_TYPE) },
          { tag: 'identity', attrs: {}, content: identityKey.public },
          (0, _index4.xmppPreKey)(key, +keyId),
          (0, _index4.xmppSignedPreKey)(signedPreKey),
  ...
  const handleEncryptNotification = async (node) => {
    ...
    {
      const identityNode = (0, _index5.getBinaryNodeChild)(node, 'identity');
      if (identityNode) {
  ...
  const handleGroupNotification = (fullNode, child, msg) => {
    ...
    switch (child?.tag) {
      ...
      case 'leave':
        ...
        const participants = (0, _index5.getBinaryNodeChildren)(child, 'participant').map(({ attrs }) => {
        ...
        // mark as left
        (0, _index5.areJidsSameUser)(participants[0].id, actingParticipantLid) ||
        (0, _index5.areJidsSameUser)(participants[0].id, actingParticipantPn)) &&
        child.tag === 'remove') {
  ...
  const processNotification = async (node) => {
    ...
    switch (nodeType) {
      ...
      case 'devices':
        ...
        if ((0, _index5.areJidsSameUser)(child.attrs.jid, authState.creds.me.id) ||
        (0, _index5.areJidsSameUser)(child.attrs.lid, authState.creds.me.lid)) {
      ...
      case 'link_code_companion_reg':
        ...
        const linkCodePairingExpanded = await (0, _index4.hkdf)(companionSharedKey, 32, {
          salt: linkCodeSalt,
        ...
        authState.creds.advSecretKey = (await (0, _index4.hkdf)(identityPayload, 32, { info: 'adv_secret' })).toString('base64');
        await query({
          tag: 'iq',
          attrs: {
  ...
  const handlePrivacyTokenNotification = async (node) => {
    ...
    if (!tokensNode)
    return;
    const tokenNodes = (0, _index5.getBinaryNodeChildren)(tokensNode, 'token');
  ...
  function toRequiredBuffer(data) {
    if (data === undefined) {
      throw new _boom.Boom('Invalid buffer', { statusCode: 400 });
    }
  ...
  const handleReceipt = async (node) => {
    ...
    if (shouldIgnoreJid(remoteJid) && remoteJid !== _index5.S_WHATSAPP_NET) {
    ...
    try {
      ...
      processingMutex.mutex(async () => {
        ...
        if (msg) {
          ...
          const { senderAlt: participantAlt, addressingMode } = (0, _index4.extractAddressingContext)(node);
          msg.key = {
            remoteJid,
            fromMe,
  ...
  const handleMessage = async (node) => {
    if (shouldIgnoreJid(node.attrs.from) && node.attrs.from !== _index5.S_WHATSAPP_NET) {
    ...
    const { fullMessage: msg, category, author, decrypt } = (0, _index4.decryptMessageNode)(node, authState.creds.me.id, authState.creds.me.lid || '', signalRepository, logger);
    ...
    try {
      await processingMutex.mutex(async () => {
        ...
        if (msg.messageStubType === _index.proto.WebMessageInfo.StubType.CIPHERTEXT && msg.category !== 'peer') {
          ...
          msg.messageStubParameters?.[0] === _index4.NO_MESSAGE_FOUND_ERROR_TEXT) {
      ...
      logger.error({ error, node: (0, _index5.binaryNodeToString)(node) }, 'error in handling message');
    }
  };
    ...
    if (call.status === 'timeout' || call.status === 'offer' && call.isGroup) {
      ...
      if (call.status === 'timeout') {
        if (call.isGroup) {
          ...
          _index3.WAMessageStubType.CALL_MISSED_GROUP_VIDEO :
          _index3.WAMessageStubType.CALL_MISSED_GROUP_VOICE;
        } else
        {
...
};exports.makeMessagesRecvSocket = makeMessagesRecvSocket; /* v9-0ad2438af7c908c0 */
