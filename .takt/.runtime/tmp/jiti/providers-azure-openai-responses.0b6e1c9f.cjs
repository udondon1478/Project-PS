"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.streamSimpleAzureOpenAIResponses = exports.streamAzureOpenAIResponses = void 0;var _openai = require("openai");
...
const streamAzureOpenAIResponses = (model, context, options) => {
  ...
  return stream;
};exports.streamAzureOpenAIResponses = streamAzureOpenAIResponses;
...
};exports.streamSimpleAzureOpenAIResponses = streamSimpleAzureOpenAIResponses;
...
function createClient(model, apiKey, options) {
  ...
  return new _openai.AzureOpenAI({
    apiKey,
    apiVersion,
...
function buildParams(model, context, options, deploymentName) {
  const messages = (0, _openaiResponsesShared.convertResponsesMessages)(model, context, AZURE_TOOL_CALL_PROVIDERS);
  const params = {
