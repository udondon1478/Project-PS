"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.formatUserTime = formatUserTime;exports.normalizeTimestamp = normalizeTimestamp;exports.resolveUserTimeFormat = resolveUserTimeFormat;exports.resolveUserTimezone = resolveUserTimezone;exports.withNormalizedTimestamp = withNormalizedTimestamp;var _nodeChild_process = require("node:child_process");
...
function withNormalizedTimestamp(value, rawTimestamp) {
  ...
  return {
    ...value,
    timestampMs: typeof value.timestampMs === "number" && Number.isFinite(value.timestampMs) ?
    value.timestampMs :
    normalized.timestampMs,
    timestampUtc: typeof value.timestampUtc === "string" && value.timestampUtc.trim() ?
    value.timestampUtc :
...
function detectSystemTimeFormat() {
  if (process.platform === "darwin") {
    try {
      const result = (0, _nodeChild_process.execSync)("defaults read -g AppleICUForce24HourTime 2>/dev/null", {
        encoding: "utf8",
        timeout: 500
      }).trim();
      if (result === "1") {
  ...
  if (process.platform === "win32") {
    try {
      const result = (0, _nodeChild_process.execSync)('powershell -Command "(Get-Culture).DateTimeFormat.ShortTimePattern"', { encoding: "utf8", timeout: 1000 }).trim();
