"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.clearPluginCommands = clearPluginCommands;exports.clearPluginCommandsForPlugin = clearPluginCommandsForPlugin;exports.executePluginCommand = executePluginCommand;exports.getPluginCommandSpecs = getPluginCommandSpecs;exports.listPluginCommands = listPluginCommands;exports.matchPluginCommand = matchPluginCommand;exports.registerPluginCommand = registerPluginCommand;exports.validateCommandName = validateCommandName;
...
const pluginCommands = new Map(); // Lock to prevent modifications during command execution
let registryLocked = false; // Maximum allowed length for command arguments (defense in depth)
...
function registerPluginCommand(pluginId, command) {
  ...
  (0, _globals.logVerbose)(`Registered plugin command: ${key} (plugin: ${pluginId})`);
  return { ok: true };
}
/**
...
async function executePluginCommand(params) {
  ...
  if (requireAuth && !isAuthorizedSender) {
    (0, _globals.logVerbose)(`Plugin command /${command.name} blocked: unauthorized sender ${senderId || "<unknown>"}`);
  ...
  try {
    ...
    (0, _globals.logVerbose)(`Plugin command /${command.name} executed successfully for ${senderId || "unknown"}`);
    return result;
  }
  catch (err) {
    const error = err;
    (0, _globals.logVerbose)(`Plugin command /${command.name} error: ${error.message}`);
