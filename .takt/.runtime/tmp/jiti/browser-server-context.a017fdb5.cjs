"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.createBrowserRouteContext = createBrowserRouteContext;var _nodeFs = _interopRequireDefault(require("node:fs"));
...
var _targetId = require("./target-id.js");
var _trash = require("./trash.js");function _interopRequireDefault(e) {return e && e.__esModule ? e : { default: e };}function _interopRequireWildcard(e, t) {if ("function" == typeof WeakMap) var r = new WeakMap(),n = new WeakMap();return (_interopRequireWildcard = function (e, t) {if (!t && e && e.__esModule) return e;var o,i,f = { __proto__: null, default: e };if (null === e || "object" != typeof e && "function" != typeof e) return f;if (o = t ? n : r) {if (o.has(e)) return o.get(e);o.set(e, f);}for (const t in e) "default" !== t && {}.hasOwnProperty.call(e, t) && ((i = (o = Object.defineProperty) && Object.getOwnPropertyDescriptor(e, t)) && (i.get || i.set) ? o(f, t, i) : f[t] = e[t]);return f;})(e, t);}
/**
...
function normalizeWsUrl(raw, cdpBaseUrl) {
  ...
  try {
    return (0, _cdp.normalizeCdpWsUrl)(raw, cdpBaseUrl);
  }
  catch {
    return raw;
  }
...
async function fetchJson(url, timeoutMs = 1500, init) {
  ...
  try {
    const headers = (0, _cdp.getHeadersWithAuth)(url, init?.headers || {});
...
function createProfileContext(opts, profile) {
  ...
  const listTabs = async () => {
    ...
    if (!profile.cdpIsLoopback) {
      const mod = await (0, _pwAiModule.getPwAiModule)({ mode: "strict" });
    ...
    const raw = await fetchJson((0, _cdp.appendCdpPath)(profile.cdpUrl, "/json/list"));
    return raw.
    map((t) => ({
      targetId: t.id ?? "",
      title: t.title ?? "",
  ...
  const openTab = async (url) => {
    ...
    const createdViaCdp = await (0, _cdp.createTargetViaCdp)({
    ...
    const endpointUrl = new URL((0, _cdp.appendCdpPath)(profile.cdpUrl, "/json/new"));
  ...
  const isReachable = async (timeoutMs) => {
    ...
    return await (0, _chrome.isChromeCdpReady)(profile.cdpUrl, httpTimeout, wsTimeout);
  };
  const isHttpReachable = async (timeoutMs) => {
    const httpTimeout = resolveRemoteHttpTimeout(timeoutMs);
    return await (0, _chrome.isChromeReachable)(profile.cdpUrl, httpTimeout);
  };
  ...
  const ensureBrowserAvailable = async () => {
    ...
    if (isExtension) {
      if (!httpReachable) {
        await (0, _extensionRelay.ensureChromeExtensionRelayServer)({ cdpUrl: profile.cdpUrl });
    ...
    if (!httpReachable) {
      ...
      const launched = await (0, _chrome.launchOpenClawChrome)(current.resolved, profile);
    ...
    await (0, _chrome.stopOpenClawChrome)(profileState.running);
    setProfileRunning(null);
    const relaunched = await (0, _chrome.launchOpenClawChrome)(current.resolved, profile);
  ...
  const ensureTabAvailable = async (targetId) => {
    ...
    const resolveById = (raw) => {
      const resolved = (0, _targetId.resolveTargetIdFromTabs)(raw, candidates);
      if (!resolved.ok) {
  ...
  const focusTab = async (targetId) => {
    const tabs = await listTabs();
    const resolved = (0, _targetId.resolveTargetIdFromTabs)(targetId, tabs);
    if (!resolved.ok) {
      if (resolved.reason === "ambiguous") {
        throw new Error("ambiguous target id prefix");
      }
    ...
    await fetchOk((0, _cdp.appendCdpPath)(profile.cdpUrl, `/json/activate/${resolved.targetId}`));
    const profileState = getProfileState();
    profileState.lastTargetId = resolved.targetId;
  };
  const closeTab = async (targetId) => {
    ...
    await fetchOk((0, _cdp.appendCdpPath)(profile.cdpUrl, `/json/close/${resolved.targetId}`));
  };
  const stopRunningBrowser = async () => {
    if (profile.driver === "extension") {
      const stopped = await (0, _extensionRelay.stopChromeExtensionRelayServer)({
        cdpUrl: profile.cdpUrl
      });
      return { stopped };
    }
  ...
  const resetProfile = async () => {
    if (profile.driver === "extension") {
      await (0, _extensionRelay.stopChromeExtensionRelayServer)({ cdpUrl: profile.cdpUrl }).catch(() => {});
    ...
    const userDataDir = (0, _chrome.resolveOpenClawUserDataDir)(profile.name);
    const profileState = getProfileState();
    ...
    try {
      ...
      // ignore
    }if (!_nodeFs.default.existsSync(userDataDir)) {
      return { moved: false, from: userDataDir };
    }
    const moved = await (0, _trash.movePathToTrash)(userDataDir);
...
function createBrowserRouteContext(opts) {
  ...
  const forProfile = (profileName) => {
    ...
    const profile = (0, _config.resolveProfile)(current.resolved, name);
    if (!profile) {
  ...
  const listProfiles = async () => {
    ...
    for (const name of Object.keys(current.resolved.profiles)) {
        ...
        }} else
      {
        ...
        try {
          const reachable = await (0, _chrome.isChromeReachable)(profile.cdpUrl, 200);
          if (reachable) {
            running = true;
            const ctx = createProfileContext(opts, profile);
