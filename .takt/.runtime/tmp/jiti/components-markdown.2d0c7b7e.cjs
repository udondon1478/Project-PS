"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.Markdown = void 0;var _marked = require("marked");
...
class Markdown {
  ...
  render(width) {
    ...
    const tokens = _marked.marked.lexer(normalizedText);
    ...
    for (const line of renderedLines) {
      ...
      {
        wrappedLines.push(...(0, _utils.wrapTextWithAnsi)(line, contentWidth));
      }
    }
    ...
    for (const line of wrappedLines) {
      ...
      if (bgFn) {
        contentLines.push((0, _utils.applyBackgroundToLine)(lineWithMargins, width, bgFn));
      } else
      {
        // No background - just pad to width
        const visibleLen = (0, _utils.visibleWidth)(lineWithMargins);
    ...
    for (let i = 0; i < this.paddingY; i++) {
      const line = bgFn ? (0, _utils.applyBackgroundToLine)(emptyLine, width, bgFn) : emptyLine;
    ...
    switch (token.type) {
      ...
      case "blockquote":{
          ...
          for (const quoteLine of quoteLines) {
            ...
            const wrappedLines = (0, _utils.wrapTextWithAnsi)(quoteLine, quoteContentWidth);
  ...
  getLongestWordWidth(text, maxWidth) {
    ...
    for (const word of words) {
      longest = Math.max(longest, (0, _utils.visibleWidth)(word));
    }
  ...
  wrapCellText(text, maxWidth) {
    return (0, _utils.wrapTextWithAnsi)(text, Math.max(1, maxWidth));
  }
  /**
  ...
  renderTable(token, availableWidth) {
    ...
    if (availableForCells < numCols) {
      ...
      const fallbackLines = token.raw ? (0, _utils.wrapTextWithAnsi)(token.raw, availableWidth) : [];
      fallbackLines.push("");
    ...
    for (let i = 0; i < numCols; i++) {
      ...
      naturalWidths[i] = (0, _utils.visibleWidth)(headerText);
      minWordWidths[i] = Math.max(1, this.getLongestWordWidth(headerText, maxUnbrokenWordWidth));
    }
    for (const row of token.rows) {
      for (let i = 0; i < row.length; i++) {
        const cellText = this.renderInlineTokens(row[i].tokens || []);
        naturalWidths[i] = Math.max(naturalWidths[i] || 0, (0, _utils.visibleWidth)(cellText));
    ...
    for (let lineIdx = 0; lineIdx < headerLineCount; lineIdx++) {
      const rowParts = headerCellLines.map((cellLines, colIdx) => {
        ...
        const padded = text + " ".repeat(Math.max(0, columnWidths[colIdx] - (0, _utils.visibleWidth)(text)));
    ...
    for (let rowIndex = 0; rowIndex < token.rows.length; rowIndex++) {
      ...
      for (let lineIdx = 0; lineIdx < rowLineCount; lineIdx++) {
        const rowParts = rowCellLines.map((cellLines, colIdx) => {
          const text = cellLines[lineIdx] || "";
          return text + " ".repeat(Math.max(0, columnWidths[colIdx] - (0, _utils.visibleWidth)(text)));
        });
        lines.push(`│ ${rowParts.join(" │ ")} │`);
      }
...
}exports.Markdown = Markdown; /* v9-d5e53235b9a09a56 */
