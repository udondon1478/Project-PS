"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.ConfigSelectorComponent = void 0;
 ...
 */const RESOURCE_TYPE_LABELS = { extensions: "Extensions",
  skills: "Skills",
  prompts: "Prompts",
  themes: "Themes"
};
...
function buildGroups(resolved) {
  ...
  const addToGroup = (resources, resourceType) => {
    for (const res of resources) {
      ...
      const fileName = (0, _nodePath.basename)(path);
      const parentFolder = (0, _nodePath.basename)((0, _nodePath.dirname)(path));
      let displayName;
...
class ConfigSelectorHeader {
  invalidate() {}
  render(width) {
    const title = _theme.theme.bold("Resource Configuration");
    const sep = _theme.theme.fg("muted", " · ");
    const hint = (0, _keybindingHints.rawKeyHint)("space", "toggle") + sep + (0, _keybindingHints.rawKeyHint)("esc", "close");
    const hintWidth = (0, _piTui.visibleWidth)(hint);
    const titleWidth = (0, _piTui.visibleWidth)(title);
    const spacing = Math.max(1, width - titleWidth - hintWidth);
    return [
    (0, _piTui.truncateToWidth)(`${title}${" ".repeat(spacing)}${hint}`, width, ""),
    _theme.theme.fg("muted", "Type to filter resources")];
...
class ResourceList {
  ...
  filterItems(query) {
    ...
    for (const entry of this.flatItems) {
      ...
      if (entry.type === "subgroup" && matchingSubgroups.has(entry.subgroup)) {
        this.filteredItems.push(entry);
      } else
      if (entry.type === "item" && matchingItems.has(entry.item)) {
        this.filteredItems.push(entry);
      }
    }
    this.selectFirstItem();
  ...
  render(width) {
    ...
    if (this.filteredItems.length === 0) {
      lines.push(_theme.theme.fg("muted", "  No resources found"));
      return lines;
    }
    ...
    for (let i = startIndex; i < endIndex; i++) {
      ...
      if (entry.type === "group") {
        ...
        const groupLine = _theme.theme.fg("accent", _theme.theme.bold(entry.group.label));
        lines.push((0, _piTui.truncateToWidth)(`  ${groupLine}`, width, ""));
      } else
      if (entry.type === "subgroup") {
        // Subgroup header (indented, no cursor)
        const subgroupLine = _theme.theme.fg("muted", entry.subgroup.label);
        lines.push((0, _piTui.truncateToWidth)(`    ${subgroupLine}`, width, ""));
      } else
      {
        // Resource item (cursor only on items)
        const item = entry.item;
        const cursor = isSelected ? "> " : "  ";
        const checkbox = item.enabled ? _theme.theme.fg("success", "[x]") : _theme.theme.fg("dim", "[ ]");
        const name = isSelected ? _theme.theme.bold(item.displayName) : item.displayName;
        lines.push((0, _piTui.truncateToWidth)(`${cursor}    ${checkbox} ${name}`, width, "..."));
      }
    }
    // Scroll indicator
    if (startIndex > 0 || endIndex < this.filteredItems.length) {
      lines.push(_theme.theme.fg("dim", `  (${this.selectedIndex + 1}/${this.filteredItems.length})`));
    }
    return lines;
  }
  handleInput(data) {
  ...
  getTopLevelBaseDir(scope) {
    return scope === "project" ? (0, _nodePath.join)(this.cwd, _config.CONFIG_DIR_NAME) : this.agentDir;
  }
  getResourcePattern(item) {
    const scope = item.metadata.scope;
    const baseDir = this.getTopLevelBaseDir(scope);
    return (0, _nodePath.relative)(baseDir, item.path);
  }
  getPackageResourcePattern(item) {
    const baseDir = item.metadata.baseDir ?? (0, _nodePath.dirname)(item.path);
    return (0, _nodePath.relative)(baseDir, item.path);
  }
}
class ConfigSelectorComponent extends _piTui.Container {
  resourceList;
  _focused = false;
  get focused() {
    return this._focused;
...
}exports.ConfigSelectorComponent = ConfigSelectorComponent; /* v9-5346fccd3bc02cd6 */
