...
var _compaction = require("./compaction.js");
 ...
 */function collectEntriesForBranchSummary(session, oldLeafId, targetId) {// If no old position, nothing to summarize
...
async function generateBranchSummary(entries, options) {
  ...
  const response = await (0, _piAi.completeSimple)(model, { systemPrompt: _utils.SUMMARIZATION_SYSTEM_PROMPT, messages: summarizationMessages }, { apiKey, signal, maxTokens: 2048 });
