"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.createProcessTool = createProcessTool;exports.processTool = void 0;var _typebox = require("@sinclair/typebox");
var _bashProcessRegistry = require("./bash-process-registry.js");
var _bashToolsShared = require("./bash-tools.shared.js");
var _ptyKeys = require("./pty-keys.js");
const processSchema = _typebox.Type.Object({
...
function createProcessTool(defaults) {
  if (defaults?.cleanupMs !== undefined) {
    (0, _bashProcessRegistry.setJobTtlMs)(defaults.cleanupMs);
  }
  ...
  return {
    ...
    execute: async (_toolCallId, args) => {
      ...
      if (params.action === "list") {
        const running = (0, _bashProcessRegistry.listRunningSessions)().
        ...
        map((s) => ({
          ...
          cwd: s.cwd,
          command: s.command,
          name: (0, _bashToolsShared.deriveSessionName)(s.command),
          tail: s.tail,
          truncated: s.truncated
        }));
        const finished = (0, _bashProcessRegistry.listFinishedSessions)().
        filter((s) => isInScope(s)).
        map((s) => ({
          sessionId: s.id,
          status: s.status,
        ...
        map((s) => {
          const label = s.name ? (0, _bashToolsShared.truncateMiddle)(s.name, 80) : (0, _bashToolsShared.truncateMiddle)(s.command, 120);
          return `${s.sessionId} ${(0, _bashToolsShared.pad)(s.status, 9)} ${(0, _bashToolsShared.formatDuration)(s.runtimeMs)} :: ${label}`;
        });
        return {
          content: [
          {
      ...
      const session = (0, _bashProcessRegistry.getSession)(params.sessionId);
      const finished = (0, _bashProcessRegistry.getFinishedSession)(params.sessionId);
      ...
      switch (params.action) {
        case "poll":{
              ...
              if (scopedFinished) {
                return {
                  ...
                  details: {
                    ...
                    name: (0, _bashToolsShared.deriveSessionName)(scopedFinished.command)
                  }
                };
              }
              return {
            ...
            const { stdout, stderr } = (0, _bashProcessRegistry.drainSession)(scopedSession);
            const exited = scopedSession.exited;
            const exitCode = scopedSession.exitCode ?? 0;
            const exitSignal = scopedSession.exitSignal ?? undefined;
            if (exited) {
              const status = exitCode === 0 && exitSignal == null ? "completed" : "failed";
              (0, _bashProcessRegistry.markExited)(scopedSession, scopedSession.exitCode ?? null, scopedSession.exitSignal ?? null, status);
            }
            const status = exited ?
            ...
            return {
              ...
              details: {
                ...
                name: (0, _bashToolsShared.deriveSessionName)(scopedSession.command)
              }
            };
          }
        case "log":{
            if (scopedSession) {
              ...
              const { slice, totalLines, totalChars } = (0, _bashToolsShared.sliceLogLines)(scopedSession.aggregated, params.offset, params.limit);
              return {
            ...
            if (scopedFinished) {
              const { slice, totalLines, totalChars } = (0, _bashToolsShared.sliceLogLines)(scopedFinished.aggregated, params.offset, params.limit);
        ...
        case "write":{
            ...
            return {
              ...
              details: {
                status: "running",
                sessionId: params.sessionId,
                name: scopedSession ? (0, _bashToolsShared.deriveSessionName)(scopedSession.command) : undefined
              }
            };
          }
        case "send-keys":{
            ...
            const { data, warnings } = (0, _ptyKeys.encodeKeySequence)({
              keys: params.keys,
              hex: params.hex,
        ...
        case "paste":{
            ...
            const payload = (0, _ptyKeys.encodePaste)(params.text ?? "", params.bracketed !== false);
            if (!payload) {
              return {
                content: [
                {
        ...
        case "kill":{
            ...
            (0, _bashToolsShared.killSession)(scopedSession);
            (0, _bashProcessRegistry.markExited)(scopedSession, null, "SIGKILL", "failed");
            return {
        ...
        case "clear":{
            if (scopedFinished) {
              (0, _bashProcessRegistry.deleteSession)(params.sessionId);
              return {
...
const processTool = exports.processTool = createProcessTool(); /* v9-e5bf4707e4af0d8a */
