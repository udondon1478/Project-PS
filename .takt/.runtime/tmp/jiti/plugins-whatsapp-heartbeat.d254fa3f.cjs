"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.resolveWhatsAppHeartbeatRecipients = resolveWhatsAppHeartbeatRecipients;var _registry = require("../../channels/registry.js");
...
function getSessionRecipients(cfg) {
  ...
  map(([_, entry]) => ({
    to: (0, _registry.normalizeChatChannelId)(entry?.lastChannel) === "whatsapp" && entry?.lastTo ?
    (0, _utils.normalizeE164)(entry.lastTo) :
    "",
...
function resolveWhatsAppHeartbeatRecipients(cfg, opts = {}) {
  if (opts.to) {
    return { recipients: [(0, _utils.normalizeE164)(opts.to)], source: "flag" };
  }
  const sessionRecipients = getSessionRecipients(cfg);
  const allowFrom = Array.isArray(cfg.channels?.whatsapp?.allowFrom) && cfg.channels.whatsapp.allowFrom.length > 0 ?
  cfg.channels.whatsapp.allowFrom.filter((v) => v !== "*").map(_utils.normalizeE164) :
  [];
