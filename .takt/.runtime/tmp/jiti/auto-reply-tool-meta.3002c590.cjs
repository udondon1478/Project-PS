"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.formatToolAggregate = formatToolAggregate;exports.formatToolPrefix = formatToolPrefix;exports.shortenMeta = shortenMeta;exports.shortenPath = shortenPath;var _toolDisplay = require("../agents/tool-display.js");
var _utils = require("../utils.js");
function shortenPath(p) {
  return (0, _utils.shortenHomePath)(p);
}
function shortenMeta(meta) {
  ...
  if (colonIdx === -1) {
    return (0, _utils.shortenHomeInString)(meta);
  }
  const base = meta.slice(0, colonIdx);
  const rest = meta.slice(colonIdx);
  return `${(0, _utils.shortenHomeInString)(base)}${rest}`;
}
function formatToolAggregate(toolName, metas, options) {
  const filtered = (metas ?? []).filter(Boolean).map(shortenMeta);
  const display = (0, _toolDisplay.resolveToolDisplay)({ name: toolName });
...
function formatToolPrefix(toolName, meta) {
  ...
  const display = (0, _toolDisplay.resolveToolDisplay)({ name: toolName, meta: extra });
  return (0, _toolDisplay.formatToolSummary)(display);
}
