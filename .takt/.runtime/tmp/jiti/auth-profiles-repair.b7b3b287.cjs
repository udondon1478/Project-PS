"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.repairOAuthProfileIdMismatch = repairOAuthProfileIdMismatch;exports.suggestOAuthProfileIdForLegacyDefault = suggestOAuthProfileIdForLegacyDefault;var _modelSelection = require("../model-selection.js");
...
function suggestOAuthProfileIdForLegacyDefault(params) {
  ...
  if (legacyCfg &&
  (0, _modelSelection.normalizeProviderId)(legacyCfg.provider) === providerKey &&
  legacyCfg.mode !== "oauth") {
    return null;
  }
  const oauthProfiles = (0, _profiles.listProfilesForProvider)(params.store, providerKey).filter((id) => params.store.profiles[id]?.type === "oauth");
...
function repairOAuthProfileIdMismatch(params) {
  const legacyProfileId = params.legacyProfileId ?? `${(0, _modelSelection.normalizeProviderId)(params.provider)}:default`;
  ...
  if ((0, _modelSelection.normalizeProviderId)(legacyCfg.provider) !== (0, _modelSelection.normalizeProviderId)(params.provider)) {
    return { config: params.cfg, changes: [], migrated: false };
  }
  ...
  const nextOrder = (() => {
    ...
    const resolvedKey = Object.keys(order).find((key) => (0, _modelSelection.normalizeProviderId)(key) === providerKey);
    if (!resolvedKey) {
      return order;
    }
