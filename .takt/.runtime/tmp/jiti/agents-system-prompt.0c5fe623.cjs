"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.buildAgentSystemPrompt = buildAgentSystemPrompt;exports.buildRuntimeLine = buildRuntimeLine;var _tokens = require("../auto-reply/tokens.js");
...
function buildMessagingSection(params) {
  ...
  `- If you use \`message\` (\`action=send\`) to deliver your user-visible reply, respond with ONLY: ${_tokens.SILENT_REPLY_TOKEN} (avoid duplicate replies).`,
...
function buildAgentSystemPrompt(params) {
  ...
  const messageChannelOptions = (0, _messageChannel.listDeliverableMessageChannels)().join("|");
  ...
  if (!isMinimal) {
    lines.push("## Silent Replies", `When you have nothing to say, respond with ONLY: ${_tokens.SILENT_REPLY_TOKEN}`, "", "⚠️ Rules:", "- It must be your ENTIRE message — nothing else", `- Never append it to an actual response (never include "${_tokens.SILENT_REPLY_TOKEN}" in real replies)`, "- Never wrap it in markdown or code blocks", "", `❌ Wrong: "Here's help... ${_tokens.SILENT_REPLY_TOKEN}"`, `❌ Wrong: "${_tokens.SILENT_REPLY_TOKEN}"`, `✅ Right: ${_tokens.SILENT_REPLY_TOKEN}`, "");
  }
