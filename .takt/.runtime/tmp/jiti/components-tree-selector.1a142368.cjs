"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.TreeSelectorComponent = void 0;var _piTui = require("@mariozechner/pi-tui");
...
class TreeList {
  ...
  render(width) {
    const lines = [];
    if (this.filteredNodes.length === 0) {
      lines.push((0, _piTui.truncateToWidth)(_theme.theme.fg("muted", "  No entries found"), width));
      lines.push((0, _piTui.truncateToWidth)(_theme.theme.fg("muted", `  (0/0)${this.getFilterLabel()}`), width));
      return lines;
    }
    ...
    for (let i = startIndex; i < endIndex; i++) {
      ...
      const pathMarker = isOnActivePath ? _theme.theme.fg("accent", "• ") : "";
      const label = flatNode.node.label ? _theme.theme.fg("warning", `[${flatNode.node.label}] `) : "";
      const content = this.getEntryDisplayText(flatNode.node, isSelected);
      let line = cursor + _theme.theme.fg("dim", prefix) + pathMarker + label + content;
      if (isSelected) {
        line = _theme.theme.bg("selectedBg", line);
      }
      lines.push((0, _piTui.truncateToWidth)(line, width));
    }
    lines.push((0, _piTui.truncateToWidth)(_theme.theme.fg("muted", `  (${this.selectedIndex + 1}/${this.filteredNodes.length})${this.getFilterLabel()}`), width));
    return lines;
  }
    ...
    switch (entry.type) {
      case "message":{
          ...
          if (role === "user") {
            ...
            result = _theme.theme.fg("accent", "user: ") + content;
          } else
          if (role === "assistant") {
            const msgWithContent = msg;
            const textContent = normalize(this.extractContent(msgWithContent.content));
            if (textContent) {
              result = _theme.theme.fg("success", "assistant: ") + textContent;
            } else
            if (msgWithContent.stopReason === "aborted") {
              result = _theme.theme.fg("success", "assistant: ") + _theme.theme.fg("muted", "(aborted)");
            } else
            if (msgWithContent.errorMessage) {
              const errMsg = normalize(msgWithContent.errorMessage).slice(0, 80);
              result = _theme.theme.fg("success", "assistant: ") + _theme.theme.fg("error", errMsg);
            } else
            {
              result = _theme.theme.fg("success", "assistant: ") + _theme.theme.fg("muted", "(no content)");
            }
          } else
          if (role === "toolResult") {
            const toolMsg = msg;
            ...
            if (toolCall) {
              result = _theme.theme.fg("muted", this.formatToolCall(toolCall.name, toolCall.arguments));
            } else
            {
              result = _theme.theme.fg("muted", `[${toolMsg.toolName ?? "tool"}]`);
            }
          } else
          if (role === "bashExecution") {
            const bashMsg = msg;
            result = _theme.theme.fg("dim", `[bash]: ${normalize(bashMsg.command ?? "")}`);
          } else
          {
            result = _theme.theme.fg("dim", `[${role}]`);
          }
          break;
        }
      case "custom_message":{
          ...
          map((c) => c.text).
          join("");
          result = _theme.theme.fg("customMessageLabel", `[${entry.customType}]: `) + normalize(content);
          break;
        }
      case "compaction":{
          const tokens = Math.round(entry.tokensBefore / 1000);
          result = _theme.theme.fg("borderAccent", `[compaction: ${tokens}k tokens]`);
          break;
        }
      case "branch_summary":
        result = _theme.theme.fg("warning", `[branch summary]: `) + normalize(entry.summary);
        break;
      case "model_change":
        result = _theme.theme.fg("dim", `[model: ${entry.modelId}]`);
        break;
      case "thinking_level_change":
        result = _theme.theme.fg("dim", `[thinking: ${entry.thinkingLevel}]`);
        break;
      case "custom":
        result = _theme.theme.fg("dim", `[custom: ${entry.customType}]`);
        break;
      case "label":
        result = _theme.theme.fg("dim", `[label: ${entry.label ?? "(cleared)"}]`);
        break;
      default:
        result = "";
    }
    return isSelected ? _theme.theme.bold(result) : result;
  }
  ...
  handleInput(keyData) {
    ...
    if ((0, _piTui.matchesKey)(keyData, "ctrl+o")) {
    ...
    if ((0, _piTui.matchesKey)(keyData, "shift+l")) {
      const selected = this.filteredNodes[this.selectedIndex];
...
class SearchLine {
  ...
  render(width) {
    ...
    if (query) {
      return [(0, _piTui.truncateToWidth)(`  ${_theme.theme.fg("muted", "Type to search:")} ${_theme.theme.fg("accent", query)}`, width)];
    }
    return [(0, _piTui.truncateToWidth)(`  ${_theme.theme.fg("muted", "Type to search:")}`, width)];
  }
...
class LabelInput {
  ...
  render(width) {
    ...
    lines.push((0, _piTui.truncateToWidth)(`${indent}${_theme.theme.fg("muted", "Label (empty to remove):")}`, width));
    lines.push(...this.input.render(availableWidth).map((line) => (0, _piTui.truncateToWidth)(`${indent}${line}`, width)));
    lines.push((0, _piTui.truncateToWidth)(`${indent}${(0, _keybindingHints.keyHint)("selectConfirm", "save")}  ${(0, _keybindingHints.keyHint)("selectCancel", "cancel")}`, width));
    return lines;
  }
  handleInput(keyData) {
    const kb = (0, _piTui.getEditorKeybindings)();
 ...
 */
class TreeSelectorComponent extends _piTui.Container {
  treeList;
  labelInput = null;
  labelInputContainer;
  treeContainer;
  ...
  constructor(tree, currentLeafId, terminalHeight, onSelect, onCancel, onLabelChange, initialSelectedId) {
    ...
    this.treeContainer = new _piTui.Container();
    this.treeContainer.addChild(this.treeList);
    this.labelInputContainer = new _piTui.Container();
    this.addChild(new _piTui.Spacer(1));
    this.addChild(new _dynamicBorder.DynamicBorder());
    this.addChild(new _piTui.Text(_theme.theme.bold("  Session Tree"), 1, 0));
    this.addChild(new _piTui.TruncatedText(_theme.theme.fg("muted", "  ↑/↓: move. ←/→: page. Shift+L: label. ") +
    _theme.theme.fg("muted", "^D/^T/^U/^L/^A: filters (^O/⇧^O cycle)"), 0, 0));
...
}exports.TreeSelectorComponent = TreeSelectorComponent; /* v9-39997cefbcd4fec7 */
