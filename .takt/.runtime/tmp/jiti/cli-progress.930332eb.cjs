"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.createCliProgress = createCliProgress;exports.withProgress = withProgress;exports.withProgressTotals = withProgressTotals;var _prompts = require("@clack/prompts");
var _oscProgress = require("osc-progress");
...
function createCliProgress(options) {
  ...
  const canOsc = isTty && (0, _oscProgress.supportsOscProgress)(process.env, isTty);
  ...
  activeProgress += 1;
  if (isTty) {
    (0, _progressLine.registerActiveProgressLine)(stream);
  }
  const controller = canOsc ?
  (0, _oscProgress.createOscProgressController)({
    env: process.env,
    isTty: stream.isTTY,
    write: (chunk) => stream.write(chunk)
  }) :
  null;
  const spin = allowSpinner ? (0, _prompts.spinner)() : null;
  ...
  () => {
    ...
    stream.write(`${_theme.theme.accent(label)}${suffix}`);
  } :
  null;
  ...
  const applyState = () => {
    ...
    if (spin) {
      spin.message(_theme.theme.accent(label));
    }
    if (renderLine) {
      renderLine();
    }
  ...
  const start = () => {
    ...
    started = true;
    if (spin) {
      spin.start(_theme.theme.accent(label));
    }
    applyState();
  };
  if (delayMs === 0) {
  ...
  const done = () => {
    ...
    (0, _progressLine.clearActiveProgressLine)();
    if (isTty) {
      (0, _progressLine.unregisterActiveProgressLine)(stream);
    }
    activeProgress = Math.max(0, activeProgress - 1);
  };
