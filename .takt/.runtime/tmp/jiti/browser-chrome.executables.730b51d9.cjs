"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.findChromeExecutableLinux = findChromeExecutableLinux;exports.findChromeExecutableMac = findChromeExecutableMac;exports.findChromeExecutableWindows = findChromeExecutableWindows;exports.resolveBrowserExecutableForPlatform = resolveBrowserExecutableForPlatform;var _nodeChild_process = require("node:child_process");
...
function execText(command, args, timeoutMs = 1200, maxBuffer = 1024 * 1024) {
  try {
    const output = (0, _nodeChild_process.execFileSync)(command, args, {
      timeout: timeoutMs,
      encoding: "utf8",
      maxBuffer
    });
...
function detectDefaultChromiumExecutableMac() {
  ...
  "read",
  _nodePath.default.join(appPath, "Contents", "Info"),
  "CFBundleExecutable"]
  );
  if (!exeName) {
    return null;
  }
  const exePath = _nodePath.default.join(appPath, "Contents", "MacOS", exeName.trim());
  if (!exists(exePath)) {
    return null;
  }
...
function detectDefaultBrowserBundleIdMac() {
  const plistPath = _nodePath.default.join(_nodeOs.default.homedir(), "Library/Preferences/com.apple.LaunchServices/com.apple.launchservices.secure.plist");
  if (!exists(plistPath)) {
    return null;
  }
...
function detectDefaultChromiumExecutableLinux() {
  ...
  const exeName = _nodePath.default.posix.basename(resolved).toLowerCase();
...
function detectDefaultChromiumExecutableWindows() {
  ...
  const exeName = _nodePath.default.win32.basename(exePath).toLowerCase();
  if (!CHROMIUM_EXE_NAMES.has(exeName)) {
    return null;
  }
...
function findDesktopFilePath(desktopId) {
  const candidates = [
  _nodePath.default.join(_nodeOs.default.homedir(), ".local", "share", "applications", desktopId),
  _nodePath.default.join("/usr/local/share/applications", desktopId),
  _nodePath.default.join("/usr/share/applications", desktopId),
  _nodePath.default.join("/var/lib/snapd/desktop/applications", desktopId)];
...
function readDesktopExecLine(desktopPath) {
  try {
    const raw = _nodeFs.default.readFileSync(desktopPath, "utf8");
...
function findChromeExecutableMac() {
  ...
  {
    kind: "chrome",
    path: _nodePath.default.join(_nodeOs.default.homedir(), "Applications/Google Chrome.app/Contents/MacOS/Google Chrome")
  },
  {
    kind: "brave",
  ...
  {
    kind: "brave",
    path: _nodePath.default.join(_nodeOs.default.homedir(), "Applications/Brave Browser.app/Contents/MacOS/Brave Browser")
  },
  {
    kind: "edge",
  ...
  {
    kind: "edge",
    path: _nodePath.default.join(_nodeOs.default.homedir(), "Applications/Microsoft Edge.app/Contents/MacOS/Microsoft Edge")
  },
  {
    kind: "chromium",
  ...
  {
    kind: "chromium",
    path: _nodePath.default.join(_nodeOs.default.homedir(), "Applications/Chromium.app/Contents/MacOS/Chromium")
  },
  {
    kind: "canary",
  ...
  {
    kind: "canary",
    path: _nodePath.default.join(_nodeOs.default.homedir(), "Applications/Google Chrome Canary.app/Contents/MacOS/Google Chrome Canary")
  }];
...
function findChromeExecutableWindows() {
  ...
  const joinWin = _nodePath.default.win32.join;
  const candidates = [];
  if (localAppData) {
    // Chrome (user install)
    candidates.push({
