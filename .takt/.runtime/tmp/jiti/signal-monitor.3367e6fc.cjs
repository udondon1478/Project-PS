"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.monitorSignalProvider = monitorSignalProvider;var _chunk = require("../auto-reply/chunk.js");
...
var _daemon = require("./daemon.js");
var _identity = require("./identity.js");
var _eventHandler = require("./monitor/event-handler.js");
var _send = require("./send.js");
var _sseReconnect = require("./sse-reconnect.js");
...
function resolveSignalReactionTargets(reaction) {
  ...
  if (author) {
    const normalized = (0, _utils.normalizeE164)(author);
...
function shouldEmitSignalReactionNotification(params) {
  ...
  if (effectiveMode === "own") {
    ...
    const normalizedAccount = (0, _utils.normalizeE164)(accountId);
  ...
  if (effectiveMode === "allowlist") {
    ...
    return (0, _identity.isSignalSenderAllowed)(sender, allowlist);
  }
  return true;
}
...
async function waitForSignalDaemonReady(params) {
  await (0, _transportReady.waitForTransportReady)({
    ...
    check: async () => {
      const res = await (0, _client.signalCheck)(params.baseUrl, 1000);
      if (res.ok) {
        return { ok: true };
      }
      return {
...
async function fetchAttachment(params) {
  ...
  const result = await (0, _client.signalRpcRequest)("getAttachment", rpcParams, {
  ...
  const saved = await (0, _store.saveMediaBuffer)(buffer, attachment.contentType ?? undefined, "inbound", params.maxBytes);
  ...
  for (const payload of replies) {
    ...
    if (mediaList.length === 0) {
      for (const chunk of (0, _chunk.chunkTextWithMode)(text, textLimit, chunkMode)) {
        await (0, _send.sendMessageSignal)(target, chunk, {
          baseUrl,
          account,
          maxBytes,
          accountId
    ...
    } else
    {
      ...
      for (const url of mediaList) {
        ...
        first = false;
        await (0, _send.sendMessageSignal)(target, caption, {
          baseUrl,
          account,
          mediaUrl: url,
          maxBytes,
...
async function monitorSignalProvider(opts = {}) {
  ...
  const textLimit = (0, _chunk.resolveTextChunkLimit)(cfg, "signal", accountInfo.accountId);
  const chunkMode = (0, _chunk.resolveChunkMode)(cfg, "signal", accountInfo.accountId);
  ...
  if (autoStart) {
    ...
    daemonHandle = (0, _daemon.spawnSignalDaemon)({
      cliPath,
      account,
      httpHost,
      httpPort,
  ...
  try {
    ...
    const handleEvent = (0, _eventHandler.createSignalEventHandler)({
      runtime,
      cfg,
      baseUrl,
      account,
    ...
    await (0, _sseReconnect.runSignalSseLoop)({
      baseUrl,
      account,
      abortSignal: opts.abortSignal,
      runtime,
