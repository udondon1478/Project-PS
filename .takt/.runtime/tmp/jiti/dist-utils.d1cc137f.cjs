"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.applyBackgroundToLine = applyBackgroundToLine;exports.extractAnsiCode = extractAnsiCode;exports.extractSegments = extractSegments;exports.getSegmenter = getSegmenter;exports.isPunctuationChar = isPunctuationChar;exports.isWhitespaceChar = isWhitespaceChar;exports.sliceByColumn = sliceByColumn;exports.sliceWithWidth = sliceWithWidth;exports.truncateToWidth = truncateToWidth;exports.visibleWidth = visibleWidth;exports.wrapTextWithAnsi = wrapTextWithAnsi;var _getEastAsianWidth = require("get-east-asian-width");
...
function graphemeWidth(segment) {
  ...
  let width = (0, _getEastAsianWidth.eastAsianWidth)(cp);
  // Trailing halfwidth/fullwidth forms
  if (segment.length > 1) {
    for (const char of segment.slice(1)) {
      const c = char.codePointAt(0);
      if (c >= 0xff00 && c <= 0xffef) {
        width += (0, _getEastAsianWidth.eastAsianWidth)(c);
      }
    }
  }
  return width;
...
function extractSegments(line, beforeEnd, afterStart, afterLen, strictAfter = false) {
  ...
  while (i < line.length) {
    ...
    if (ansi) {
      ...
      if (currentCol >= afterStart && currentCol < afterEnd && afterStarted) {
    ...
    for (const { segment } of segmenter.segment(line.slice(i, textEnd))) {
      ...
      if (currentCol >= afterStart && currentCol < afterEnd) {
