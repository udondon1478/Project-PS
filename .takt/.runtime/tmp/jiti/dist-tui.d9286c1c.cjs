"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.TUI = exports.Container = exports.CURSOR_MARKER = void 0;exports.isFocusable = isFocusable;Object.defineProperty(exports, "visibleWidth", { enumerable: true, get: function () {return _utils.visibleWidth;} });
...
var _terminalImage = require("./terminal-image.js");
 ...
 */ /** Type guard to check if a component implements Focusable */function isFocusable(component) {
 ...
 */
const CURSOR_MARKER = exports.CURSOR_MARKER = "\x1b_pi:c\x07";
...
class TUI extends Container {
  ...
  queryCellSize() {
    ...
    if (!(0, _terminalImage.getCapabilities)().images) {
      return;
    }
  ...
  handleInput(data) {
    ...
    if ((0, _keys.matchesKey)(data, "shift+ctrl+d") && this.onDebug) {
      this.onDebug();
      return;
    }
    ...
    if (this.focusedComponent?.handleInput) {
      ...
      if ((0, _keys.isKeyRelease)(data) && !this.focusedComponent.wantsKeyRelease) {
        return;
      }
  ...
  parseCellSizeResponse() {
    ...
    if (match) {
      ...
      if (heightPx > 0 && widthPx > 0) {
        (0, _terminalImage.setCellDimensions)({ widthPx, heightPx });
  ...
  compositeOverlays(lines, termWidth, termHeight) {
    ...
    for (const { overlayLines, row, col, w } of rendered) {
      for (let i = 0; i < overlayLines.length; i++) {
        ...
        if (idx >= 0 && idx < result.length) {
          ...
          const truncatedOverlayLine = (0, _utils.visibleWidth)(overlayLines[i]) > w ? (0, _utils.sliceByColumn)(overlayLines[i], 0, w, true) : overlayLines[i];
    ...
    for (const idx of modifiedLines) {
      const lineWidth = (0, _utils.visibleWidth)(result[idx]);
      if (lineWidth > termWidth) {
        result[idx] = (0, _utils.sliceByColumn)(result[idx], 0, termWidth, true);
      }
    }
    return result;
  }
  ...
  applyLineResets(lines) {
    ...
    for (let i = 0; i < lines.length; i++) {
      const line = lines[i];
      if (!(0, _terminalImage.isImageLine)(line)) {
        lines[i] = line + reset;
      }
    }
    return lines;
  ...
  compositeLineAt(baseLine, overlayLine, startCol, overlayWidth, totalWidth) {
    if ((0, _terminalImage.isImageLine)(baseLine))
    return baseLine;
    // Single pass through baseLine extracts both before and after segments
    const afterStart = startCol + overlayWidth;
    const base = (0, _utils.extractSegments)(baseLine, startCol, afterStart, totalWidth - afterStart, true);
    // Extract overlay with width tracking (strict=true to exclude wide chars at boundary)
    const overlay = (0, _utils.sliceWithWidth)(overlayLine, 0, overlayWidth, true);
    ...
    const resultWidth = (0, _utils.visibleWidth)(result);
    if (resultWidth <= totalWidth) {
      return result;
    }
    // Truncate with strict=true to ensure we don't exceed totalWidth
    return (0, _utils.sliceByColumn)(result, 0, totalWidth, true);
  }
  /**
  ...
  extractCursorPosition(lines, height) {
    ...
    for (let row = lines.length - 1; row >= viewportTop; row--) {
      ...
      if (markerIndex !== -1) {
        ...
        const col = (0, _utils.visibleWidth)(beforeMarker);
  ...
  doRender() {
    ...
    for (let i = firstChanged; i <= renderEnd; i++) {
      ...
      const isImage = (0, _terminalImage.isImageLine)(line);
      if (!isImage && (0, _utils.visibleWidth)(line) > width) {
        // Log all lines to crash file for debugging
        const crashLogPath = path.join(os.homedir(), ".pi", "agent", "pi-crash.log");
        const crashData = [
        `Crash at ${new Date().toISOString()}`,
        `Terminal width: ${width}`,
        `Line ${i} visible width: ${(0, _utils.visibleWidth)(line)}`,
        "",
        "=== All rendered lines ===",
        ...newLines.map((l, idx) => `[${idx}] (w=${(0, _utils.visibleWidth)(l)}) ${l}`),
        ""].
        join("\n");
        ...
        this.stop();
        const errorMsg = [
        `Rendered line ${i} exceeds terminal width (${(0, _utils.visibleWidth)(line)} > ${width}).`,
        "",
...
}exports.TUI = TUI; /* v9-9e5d60409d3f32f5 */
