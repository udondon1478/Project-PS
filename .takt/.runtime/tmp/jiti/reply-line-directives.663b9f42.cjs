"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.hasLineDirectives = hasLineDirectives;exports.parseLineDirectives = parseLineDirectives;var _flexTemplates = require("../../line/flex-templates.js");
/**
...
function parseLineDirectives(payload) {
  ...
  if (mediaPlayerMatch && !lineData.flexMessage) {
    ...
    if (parts.length >= 1) {
      ...
      const card = (0, _flexTemplates.createMediaPlayerCard)({
  ...
  if (eventMatch && !lineData.flexMessage) {
    ...
    if (parts.length >= 2) {
      ...
      const card = (0, _flexTemplates.createEventCard)({
        title: title || "Event",
        date: date || "TBD",
        time: time || undefined,
  ...
  if (appleTvMatch && !lineData.flexMessage) {
    ...
    if (parts.length >= 1) {
      ...
      const card = (0, _flexTemplates.createAppleTvRemoteCard)({
  ...
  if (agendaMatch && !lineData.flexMessage) {
    ...
    if (parts.length >= 2) {
      ...
      const card = (0, _flexTemplates.createAgendaCard)({
        title: title || "Agenda",
        events
      });
      lineData.flexMessage = {
  ...
  if (deviceMatch && !lineData.flexMessage) {
    ...
    if (parts.length >= 1) {
      ...
      [];
      const card = (0, _flexTemplates.createDeviceControlCard)({
