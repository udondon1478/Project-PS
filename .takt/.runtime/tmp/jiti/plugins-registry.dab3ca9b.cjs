"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.createPluginRegistry = createPluginRegistry;var _nodePath = _interopRequireDefault(require("node:path"));
var _internalHooks = require("../hooks/internal-hooks.js");
var _utils = require("../utils.js");
var _commands = require("./commands.js");
var _httpPath = require("./http-path.js");function _interopRequireDefault(e) {return e && e.__esModule ? e : { default: e };}
function createPluginRegistry(registryParams) {
  ...
  const registerHook = (record, events, handler, opts, config) => {
    ...
    for (const event of normalizedEvents) {
      (0, _internalHooks.registerInternalHook)(event, handler);
    }
  };
  ...
  const registerHttpRoute = (record, params) => {
    const normalizedPath = (0, _httpPath.normalizePluginHttpPath)(params.path);
    if (!normalizedPath) {
      pushDiagnostic({
        level: "warn",
        pluginId: record.id,
  ...
  const registerCommand = (record, command) => {
    ...
    const result = (0, _commands.registerPluginCommand)(record.id, command);
    if (!result.ok) {
      pushDiagnostic({
        level: "error",
        pluginId: record.id,
  ...
  const createApi = (record, params) => {
    return {
      ...
      resolvePath: (input) => (0, _utils.resolveUserPath)(input),
