"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.encodeWAM = void 0;var _BinaryInfo = require("./BinaryInfo.js");
...
function encodeGlobalAttributes(binaryInfo, globals) {
  for (const [key, _value] of Object.entries(globals)) {
    const id = _constants.WEB_GLOBALS.find((a) => a?.name === key).id;
    let value = _value;
    if (typeof value === 'boolean') {
      value = value ? 1 : 0;
    }
    binaryInfo.buffer.push(serializeData(id, value, _constants.FLAG_GLOBAL));
  }
}
function encodeEvents(binaryInfo) {
  for (const [name, { props, globals }] of binaryInfo.events.map((a) => Object.entries(a)[0])) {
    encodeGlobalAttributes(binaryInfo, globals);
    const event = _constants.WEB_EVENTS.find((a) => a.name === name);
    const props_ = Object.entries(props);
    let extended = false;
    for (const [, value] of props_) {
      extended || (extended = value !== null);
    }
    const eventFlag = extended ? _constants.FLAG_EVENT : _constants.FLAG_EVENT | _constants.FLAG_EXTENDED;
    ...
    for (let i = 0; i < props_.length; i++) {
      ...
      const fieldFlag = extended ? _constants.FLAG_EVENT : _constants.FLAG_FIELD | _constants.FLAG_EXTENDED;
...
function serializeData(key, value, flag) {
  ...
  let offset = 0;
  if (value === null) {
    if (flag === _constants.FLAG_GLOBAL) {
...
function serializeHeader(buffer, offset, key, flag) {
  ...
  {
    buffer.writeUInt8(flag | _constants.FLAG_BYTE, offset);
    offset += 1;
