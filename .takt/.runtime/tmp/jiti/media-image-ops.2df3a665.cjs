"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.convertHeicToJpeg = convertHeicToJpeg;exports.getImageMetadata = getImageMetadata;exports.hasAlphaChannel = hasAlphaChannel;exports.normalizeExifOrientation = normalizeExifOrientation;exports.optimizeImageToPng = optimizeImageToPng;exports.resizeToJpeg = resizeToJpeg;exports.resizeToPng = resizeToPng;var _promises = _interopRequireDefault(require("node:fs/promises"));
var _nodeOs = _interopRequireDefault(require("node:os"));
var _nodePath = _interopRequireDefault(require("node:path"));
var _exec = require("../process/exec.js");function _interopRequireDefault(e) {return e && e.__esModule ? e : { default: e };}function _interopRequireWildcard(e, t) {if ("function" == typeof WeakMap) var r = new WeakMap(),n = new WeakMap();return (_interopRequireWildcard = function (e, t) {if (!t && e && e.__esModule) return e;var o,i,f = { __proto__: null, default: e };if (null === e || "object" != typeof e && "function" != typeof e) return f;if (o = t ? n : r) {if (o.has(e)) return o.get(e);o.set(e, f);}for (const t in e) "default" !== t && {}.hasOwnProperty.call(e, t) && ((i = (o = Object.defineProperty) && Object.getOwnPropertyDescriptor(e, t)) && (i.get || i.set) ? o(f, t, i) : f[t] = e[t]);return f;})(e, t);}
function isBun() {
...
async function sipsMetadataFromBuffer(buffer) {
  return await withTempDir(async (dir) => {
    ...
    const { stdout } = await (0, _exec.runExec)("/usr/bin/sips", ["-g", "pixelWidth", "-g", "pixelHeight", input], {
      timeoutMs: 10_000,
      maxBuffer: 512 * 1024
    });
...
async function sipsResizeToJpeg(params) {
  return await withTempDir(async (dir) => {
    ...
    await (0, _exec.runExec)("/usr/bin/sips", [
    "-Z",
...
async function sipsConvertToJpeg(buffer) {
  return await withTempDir(async (dir) => {
    ...
    const output = _nodePath.default.join(dir, "out.jpg");
    await _promises.default.writeFile(input, buffer);
    await (0, _exec.runExec)("/usr/bin/sips", ["-s", "format", "jpeg", input, "--out", output], {
      timeoutMs: 20_000,
      maxBuffer: 1024 * 1024
    });
    return await _promises.default.readFile(output);
...
async function sipsApplyOrientation(buffer, orientation) {
  ...
  return await withTempDir(async (dir) => {
    ...
    const output = _nodePath.default.join(dir, "out.jpg");
    await _promises.default.writeFile(input, buffer);
    await (0, _exec.runExec)("/usr/bin/sips", [...ops, input, "--out", output], {
      timeoutMs: 20_000,
      maxBuffer: 1024 * 1024
    });
    return await _promises.default.readFile(output);
