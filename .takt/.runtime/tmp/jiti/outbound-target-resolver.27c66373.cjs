"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.formatTargetDisplay = formatTargetDisplay;exports.lookupDirectoryDisplay = lookupDirectoryDisplay;exports.resetDirectoryCache = resetDirectoryCache;exports.resolveChannelTarget = resolveChannelTarget;exports.resolveMessagingTarget = resolveMessagingTarget;var _index = require("../../channels/plugins/index.js");
var _runtime = require("../../runtime.js");
var _directoryCache = require("./directory-cache.js");
...
const directoryCache = new _directoryCache.DirectoryCache(CACHE_TTL_MS);
...
function normalizeDirectoryEntryId(channel, entry) {
  const normalized = (0, _targetNormalization.normalizeTargetForProvider)(channel, entry.id);
...
async function getDirectoryEntries(params) {
  const signature = (0, _targetNormalization.buildTargetResolverSignature)(params.channel);
  const cacheKey = (0, _directoryCache.buildDirectoryCacheKey)({
  ...
  const liveKey = (0, _directoryCache.buildDirectoryCacheKey)({
    channel: params.channel,
    accountId: params.accountId,
    kind: params.kind,
    source: "live",
...
async function resolveMessagingTarget(params) {
  const raw = (0, _targetNormalization.normalizeChannelTargetInput)(params.input);
  if (!raw) {
  ...
  const normalized = (0, _targetNormalization.normalizeTargetForProvider)(params.channel, raw) ?? raw;
  ...
  if (match.kind === "ambiguous") {
    ...
    return {
      ok: false,
      error: (0, _targetErrors.ambiguousTargetError)(providerLabel, raw, hint),
  ...
  return {
    ok: false,
    error: (0, _targetErrors.unknownTargetError)(providerLabel, raw, hint)
  };
}
async function lookupDirectoryDisplay(params) {
  const normalized = (0, _targetNormalization.normalizeTargetForProvider)(params.channel, params.targetId) ?? params.targetId;
