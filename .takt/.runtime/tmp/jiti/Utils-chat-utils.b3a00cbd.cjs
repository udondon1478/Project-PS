"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.processSyncAction = exports.newLTHashState = exports.extractSyncdPatches = exports.encodeSyncdPatch = exports.downloadExternalPatch = exports.downloadExternalBlob = exports.decodeSyncdSnapshot = exports.decodeSyncdPatch = exports.decodeSyncdMutations = exports.decodePatches = exports.chatModificationToAppPatch = void 0;var _boom = require("@hapi/boom");
...
const mutationKeys = async (keydata) => {
  const expanded = await (0, _crypto.hkdf)(keydata, 160, { info: 'WhatsApp Mutation Keys' });
  return {
...
const generateMac = (operation, data, keyId, key) => {
  const getKeyData = () => {
    let r;
    switch (operation) {
      case _index.proto.SyncdMutation.SyncdOperation.SET:
        r = 0x01;
        break;
      case _index.proto.SyncdMutation.SyncdOperation.REMOVE:
        r = 0x02;
        break;
    }
  ...
  return {
    mix: ({ indexMac, valueMac, operation }) => {
      ...
      if (operation === _index.proto.SyncdMutation.SyncdOperation.REMOVE) {
        if (!prevOp) {
          throw new _boom.Boom('tried remove, but no previous op', { data: { indexMac, valueMac } });
        }
    ...
    finish: async () => {
      ...
      const result = await _ltHash.LT_HASH_ANTI_TAMPERING.subtractThenAdd(hashArrayBuffer, addBuffs, subBuffs);
...
const newLTHashState = () => ({ version: 0, hash: Buffer.alloc(128), indexValueMap: {} });exports.newLTHashState = newLTHashState;
const encodeSyncdPatch = async ({ type, index, syncAction, apiVersion, operation }, myAppStateKeyId, state, getAppStateSyncKey) => {
  const key = !!myAppStateKeyId ? await getAppStateSyncKey(myAppStateKeyId) : undefined;
  if (!key) {
    throw new _boom.Boom(`myAppStateKey ("${myAppStateKeyId}") not present`, { statusCode: 404 });
  }
  ...
  for (const msgMutation of msgMutations) {
    ...
    if (validateMacs) {
      ...
      if (Buffer.compare(contentHmac, ogValueMac) !== 0) {
        throw new _boom.Boom('HMAC content verification failed');
      }
    }
    const result = (0, _crypto.aesDecrypt)(encContent, key.valueEncryptionKey);
...
const decodeSyncdPatch = async (msg, name, initialState, getAppStateSyncKey, onMutation, validateMacs) => {
  if (validateMacs) {
    ...
    if (!mainKeyObj) {
      throw new _boom.Boom(`failed to find key "${base64Key}" to decode patch`, { statusCode: 404, data: { msg } });
    }
...
const downloadExternalBlob = async (blob, options) => {
  const stream = await (0, _messagesMedia.downloadContentFromMessage)(blob, 'md-app-state', { options });
  const bufferArray = [];
...
const decodeSyncdSnapshot = async (name, snapshot, getAppStateSyncKey, minimumVersionNumber, validateMacs = true) => {
  ...
  if (validateMacs) {
    ...
    if (Buffer.compare(snapshot.mac, computedSnapshotMac) !== 0) {
      throw new _boom.Boom(`failed to verify LTHash at ${newState.version} of ${name} from snapshot`);
    }
  }
  return {
    state: newState,
  ...
  for (const syncd of syncds) {
    ...
    if (validateMacs) {
      ...
      if (Buffer.compare(snapshotMac, computedSnapshotMac) !== 0) {
        throw new _boom.Boom(`failed to verify LTHash at ${newState.version} of ${name}`);
      }
    }
...
const chatModificationToAppPatch = (mod, jid) => {
      ...
      messageRange = {
        ...
        lastMessages.map((m) => {
          if (!m.key?.id || !m.key?.remoteJid) {
            throw new _boom.Boom('Incomplete key', { statusCode: 400, data: m });
          }
          if ((0, _index2.isJidGroup)(m.key.remoteJid) && !m.key.fromMe && !m.key.participant) {
            throw new _boom.Boom('Expected not from me message to have participant', { statusCode: 400, data: m });
          }
          if (!m.messageTimestamp || !(0, _generics.toNumber)(m.messageTimestamp)) {
            throw new _boom.Boom('Missing timestamp in last message list', { statusCode: 400, data: m });
          }
  ...
  if ('removeMessageLabel' in mod) {
    patch = {
      ...
      index: [
      _LabelAssociation.LabelAssociationType.Message,
...
const processSyncAction = (syncAction, ev, me, initialSyncOpts, logger) => {
  ...
  if (action?.archiveChatAction || type === 'archive' || type === 'unarchive') {
  ...
  if (action?.deleteMessageForMeAction || type === 'deleteMessageForMe') {
  ...
  if (action?.starAction || type === 'star') {
  ...
  if (action?.deleteChatAction || type === 'deleteChat') {
    if (!isInitialSync) {
      ev.emit('chats.delete', [id]);
    }
  } else
  if (action?.labelEditAction) {
  ...
  if (action?.labelAssociationAction) {
...
};exports.processSyncAction = processSyncAction; /* v9-6e21fa756f21f90c */
