...
export default function RegisterItemPage() {
  ...
  const { data: _session, status: sessionStatus } = useSession(); // useSession を呼び出し

  const isLoading = status === 'loading' || sessionStatus === 'loading'; // ローディング状態にセッション読み込みを追加
    ...
    try {
      const response = await fetch('/api/items', {
        ...
        headers: {
          ...
          // セッション情報（例: トークン）をヘッダーに追加
          ...(_session?.accessToken && { Authorization: `Bearer ${_session.accessToken}` }),
        },
  ...
  return (
    <div style={{ padding: '20px' }}>
      ...
      {/* セッション読み込み中 */}
      {sessionStatus === 'loading' && <p>セッション情報を読み込み中...</p>}

      {/* 認証されていない場合 */}
      {sessionStatus === 'unauthenticated' && (
        <p style={{ color: 'red' }}>商品登録にはログインが必要です。</p>
      )}
