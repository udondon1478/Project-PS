...
import { useRouter, useSearchParams, usePathname } from 'next/navigation'; // useRouterとuseSearchParamsを追加
...
export default function ProductSearch({
  ...
  const handleSearch = useCallback(() => {
    ...
    if (pathname === '/profile/likes') {
      queryParams.append('filter', 'liked');
    } else if (pathname === '/profile/owned') {
      queryParams.append('filter', 'owned');
    }

    router.replace(`/search?${queryParams.toString()}`);
  }, [selectedTags, selectedNegativeTags, selectedAgeRatingTags, detailedFilters, priceRange, isHighPriceFilterEnabled, router, pathname]); // 依存配列にisHighPriceFilterEnabledとselectedAgeRatingTagsを追加
