...
interface ProductDetailClientPageProps {
  ...
  initialTagMap: { [key: string]: string };
}

const ProductDetailClientPage: React.FC<ProductDetailClientPageProps> = ({ initialProduct, initialTagMap }) => {
  const [product, setProduct] = useState<ProductDetail>(initialProduct);
  const [tagMap, setTagMap] = useState<{ [key: string]: string }>(initialTagMap);
  ...
  // Function to refetch product data after updates
  ...
  const handleLikeToggle = async () => {
    ...
    try {
      const response = await fetch(`/api/products/${product.id}/like`, { method: !originalIsLiked ? 'POST' : 'DELETE' });
  ...
  const handleOwnToggle = async () => {
    ...
    try {
      const response = await fetch(`/api/products/${product.id}/own`, { method: !originalIsOwned ? 'POST' : 'DELETE' });
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12">
            ...
            <div className="mb-6">
              ...
              {product.seller && (
                <a href={product.seller.sellerUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center space-x-2 text-sm text-gray-600 dark:text-gray-300 hover:underline">
                  <Image src={product.seller.iconUrl || '/pslogo.svg'} alt={product.seller.name} width={28} height={28} className="rounded-full" />
          ...
          <aside className="lg:col-span-4">
            <div className="sticky top-32 space-y-6">
              ...
              <div id="onboarding-tag-list" className="p-6 bg-slate-50 dark:bg-slate-800/50 rounded-lg border dark:border-slate-700">
                <div className="flex justify-between items-center mb-4">
                  <h2 className="text-xl font-semibold">タグ</h2>
                  <Dialog open={isTagEditorOpen} onOpenChange={setIsTagEditorOpen}>
                    <DialogTrigger asChild>
                      <Button id="onboarding-edit-tags-button" variant="outline" size="sm">編集</Button>
                    </DialogTrigger>
                  ...
                  {product.productTags && product.productTags.length > 0 ? (
                    <div className="flex flex-col">
                      ...
                      <div className="mt-4 flex-shrink-0">
                        <Dialog>
                          <DialogTrigger asChild>
                            <Button id="onboarding-tag-history-button" variant="outline" className="w-full" disabled={!product.tagEditHistory || product.tagEditHistory.length === 0}>
                              タグ編集履歴を閲覧
                            </Button>
                          </DialogTrigger>
                              ...
                              <ScrollArea className="h-full">
                                <div className="space-y-4 pr-6">
                                  {product.tagEditHistory?.length > 0 ? (
                                    product.tagEditHistory.map((history, index) => (
                                      <TagEditHistoryItem key={history.id} history={history} tagMap={tagMap} isFirst={index === 0} />
                                    ))
                                  ) : (
                                    <p>編集履歴はありません。</p>
                                  )}
